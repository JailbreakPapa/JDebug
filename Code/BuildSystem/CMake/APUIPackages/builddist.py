...
def ProcessOptions(args):
	...
	options = {'APUI_VERSION': 'custom', 'FULL_SOURCE': False, 'ARCHIVE_NAME': 'APUI-sdk', 'SAMPLE_BINARIES': False, 'ARCHIVE': False}
	...
	for opt in optlist:
		if opt[0] == '-h':
			Usage(args)
		if opt[0] == '-v':
			options['APUI_VERSION'] = opt[1]
		if opt[0] == '-s':
			options['FULL_SOURCE'] = True
			options['ARCHIVE_NAME'] = 'APUI-source'
		if opt[0] == '-b':
...
def main():
	...
	#CheckVSVars()
	#Build('APUICore', ['Debug', 'Release'], {'APUI_VERSION': '\\"' + options['APUI_VERSION'] + '\\"'})
	#Build('APUIControls', ['Debug', 'Release'])
	#Build('APUIDebugger', ['Debug', 'Release'])
	
	DelTree('../Distribution/APUI')
	CopyFiles('../Include', '../Distribution/APUI/Include')
	CopyFiles('../Build', '../Distribution/APUI/Build', ['\.dll$', '^APUI.*\.lib$'], ['CMakeFiles'])
	CopyFiles('../CMake', '../Distribution/APUI/CMake', ['\.cmake$', '\.in$', '\.plist$', '\.py$', '\.sh$'])
	CopyFiles('../Samples', '../Distribution/APUI/Samples', ['\.h$', '\.cpp$', '\.rml$', '\.rcss$', '\.tga$', '\.py$', '\.otf$', '\.ttf$', '\.txt$'])
	if options['FULL_SOURCE']:
		CopyFiles('../Build', '../Distribution/APUI/Build', ['\.vcxproj$', '\.sln$', '\.vsprops$', '\.py$'], ['CMakeFiles'])
		CopyFiles('../Source', '../Distribution/APUI/Source', ['\.cpp$', '\.h$', '\.inl$'])
	if options['SAMPLE_BINARIES']:
		CopyFiles('../Build', '../Distribution/APUI/Build', ['\.exe$'], ['CMakeFiles'])
	shutil.copyfile('../LICENSE', '../Distribution/APUI/LICENSE')
	shutil.copyfile('../readme.md', '../Distribution/APUI/readme.md')
	if options['ARCHIVE']:
		Archive(options['ARCHIVE_NAME'] + '-' + options['APUI_VERSION'], '../Distribution/APUI');
