...
function(ns_check_build_type)
	# set the default build type
	if(NOT CMAKE_BUILD_TYPE)
		set(CMAKE_BUILD_TYPE ${NS_BUILDTYPENAME_DEV} CACHE STRING "Choose the type of build, options are: None ${NS_BUILDTYPENAME_DEBUG} ${NS_BUILDTYPENAME_DEV} ${NS_BUILDTYPENAME_RELEASE}." FORCE)
	endif()
endfunction()

# #####################################
# ## ns_set_build_flags_msvc(<target>)
# #####################################
function(ns_set_build_flags_msvc TARGET_NAME)
	...
	if((CMAKE_SIZEOF_VOID_P EQUAL 4) AND NS_CMAKE_ARCHITECTURE_X86)
	...
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_RELEASE_UPPER}>:/Zo>")
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_DEV_UPPER}>:/Zo>")

	# /Ob1: Only consider functions for inlining that are marked with inline or forceinline
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_DEBUG_UPPER}>:/Ob1>")

	# /Ox: favor speed for optimizations
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_RELEASE_UPPER}>:/Ox>")

	# /Ob2: Consider all functions for inlining
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_RELEASE_UPPER}>:/Ob2>")

	# /Oi: Replace some functions with intrinsics or other special forms of the function
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${NS_BUILDTYPENAME_RELEASE_UPPER}>:/Oi>")
	...
	if(CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND NS_CMAKE_ARCHITECTURE_X86)
	...
	if(${ARG_NO_DEBUG})
		...
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${NS_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_RELEASE})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${NS_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_RELEASE})
	else()
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${NS_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_DEBUG})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${NS_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_${NS_DEV_BUILD_LINKERFLAGS}})
	endif()

	set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${NS_BUILDTYPENAME_RELEASE_UPPER} ${LINKER_FLAGS_RELEASE})

	if(NS_ENABLE_COMPILER_STATIC_ANALYSIS)
...
function(ns_set_build_flags_clang TARGET_NAME)
	...
	if(NS_ENABLE_QT_SUPPORT)
		...
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${NS_QT_DIR}\"")
	endif()

	# Ignore any warnings caused by headers inside the ThirdParty directory.
	if(NS_SUBMODULE_PREFIX_PATH)
...
# #####################################
# ## ns_set_build_flags_gcc(<target>)
# #####################################
function(ns_set_build_flags_gcc TARGET_NAME)
	...
	if(NOT(CMAKE_CURRENT_SOURCE_DIR MATCHES "Code/ThirdParty"))
		# Warning / Error settings for ns code
		# attributes = error if a attribute is placed incorrectly (e.g. NS_FOUNDATION_DLL)
		# unused-result = error if [[nodiscard]] return value is not handeled (nsResult)
...
# #####################################
# ## ns_set_build_flags(<target>)
# #####################################
function(ns_set_build_flags TARGET_NAME)
	...
	if(NS_CMAKE_COMPILER_MSVC)
		ns_set_build_flags_msvc(${TARGET_NAME} ${ARGN})
	endif()

	if(NS_CMAKE_COMPILER_CLANG)
		ns_set_build_flags_clang(${TARGET_NAME} ${ARGN})
	endif()

	if(NS_CMAKE_COMPILER_GCC)
		ns_set_build_flags_gcc(${TARGET_NAME} ${ARGN})
	endif()
endfunction()
