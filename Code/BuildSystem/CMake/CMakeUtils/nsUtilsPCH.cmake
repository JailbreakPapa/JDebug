...
# #####################################
# ## ns_retrieve_target_pch(<target> <pch-name>)
# #####################################
function(ns_retrieve_target_pch TARGET_NAME PCH_NAME)
...
# #####################################
# ## ns_pch_use(<pch-header> <cpp-files>)
# #####################################
function(ns_pch_use PCH_H TARGET_CPPS)
	if(NOT NS_USE_PCH OR NS_CMAKE_GENERATOR_MSVC)
...
# #####################################
# ## ns_pch_create(<pch-header> <cpp-file>)
# #####################################
function(ns_pch_create PCH_H TARGET_CPP)
	if(NOT NS_CMAKE_GENERATOR_MSVC)
		return()
	endif()
...
# #####################################
# ## ns_find_pch_in_file_list(<files> <out-pch-name>)
# #####################################
function(ns_find_pch_in_file_list FILE_LIST PCH_PATH PCH_NAME)
...
# #####################################
# ## ns_auto_pch(<target> <files> [<file-exclude-regex> ... ])
# #####################################
function(ns_auto_pch TARGET_NAME FILES)
	foreach(EXCLUDE_PATTERN ${ARGN})
		list(FILTER FILES EXCLUDE REGEX ${EXCLUDE_PATTERN})
	endforeach()

	ns_find_pch_in_file_list("${FILES}" PCH_PATH PCH_NAME)
	
	if(NOT PCH_NAME)
		return()
	endif()

	ns_pch_create("${PCH_NAME}.h" ${PCH_PATH})

	ns_pch_use("${PCH_NAME}.h" "${FILES}")

	ns_set_target_pch(${TARGET_NAME} ${PCH_NAME})
endfunction()
