# #####################################
# ## ns_add_pvs_header(<target-var> <optional-text>)
# #####################################

function(ns_add_pvs_header TARGET_VAR OPTIONAL_TEXT)
	if(NS_ENABLE_PVS_STUDIO_HEADER_IN_UNITY_FILES)
...
# #####################################
# ## ns_generate_folder_unity_file(<target> <project-path> <rel-path-to-folder> <files-in-folder>)
# #####################################
function(ns_generate_folder_unity_file TARGET_NAME PROJECT_DIRECTORY SUB_FOLDER_PATH FILE_LIST)
	if(NOT ${NS_ENABLE_FOLDER_UNITY_FILES})
		return()
	endif()
	...
	ns_add_pvs_header(UNITY_FILE_CONTENTS "// generated unity cpp file\n\n")

	# find the PCH and include it as the first thing in the unity file
	ns_retrieve_target_pch(${TARGET_NAME} PCH_FILE)

	if(PCH_FILE)
		string(APPEND UNITY_FILE_CONTENTS "#include <${PCH_FILE}.h>\n\n")

		ns_pch_use("${PCH_FILE}.h" "${FOLDER_UNITY_FILE}")
	endif()
...
# #####################################
# ## ns_generate_folder_unity_files_for_target(<target> <project-path> [<folder-to-exclude> ...])
# #####################################
function(ns_generate_folder_unity_files_for_target TARGET_NAME TARGET_DIRECTORY)
	ns_gather_subfolders(${TARGET_DIRECTORY} ALL_FOLDERS)
	...
	foreach(CUR_FOLDER ${ALL_FOLDERS})
		...
		ns_generate_folder_unity_file(${TARGET_NAME} "${TARGET_DIRECTORY}" "${CUR_FOLDER}" "${FILES_IN_FOLDER}")
	endforeach()
endfunction()