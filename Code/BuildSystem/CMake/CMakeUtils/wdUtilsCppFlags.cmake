...
function(wd_check_build_type)
	# set the default build type
	if(NOT CMAKE_BUILD_TYPE)
		set(CMAKE_BUILD_TYPE ${WD_BUILDTYPENAME_DEV} CACHE STRING "Choose the type of build, options are: None ${WD_BUILDTYPENAME_DEBUG} ${WD_BUILDTYPENAME_DEV} ${WD_BUILDTYPENAME_RELEASE}." FORCE)
	endif()
endfunction()

# #####################################
# ## wd_set_build_flags_msvc(<target>)
# #####################################
function(wd_set_build_flags_msvc TARGET_NAME)
	...
	if((CMAKE_SIZEOF_VOID_P EQUAL 4) AND WD_CMAKE_ARCHITECTURE_X86)
	...
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_RELEASE_UPPER}>:/Zo>")
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_DEV_UPPER}>:/Zo>")

	# /Ob1: Only consider functions for inlining that are marked with inline or forceinline
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_DEBUG_UPPER}>:/Ob1>")

	# /Ox: favor speed for optimizations
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_RELEASE_UPPER}>:/Ox>")

	# /Ob2: Consider all functions for inlining
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_RELEASE_UPPER}>:/Ob2>")

	# /Oi: Replace some functions with intrinsics or other special forms of the function
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${WD_BUILDTYPENAME_RELEASE_UPPER}>:/Oi>")
	...
	if(CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND WD_CMAKE_ARCHITECTURE_X86)
	...
	if(${ARG_NO_DEBUG})
		...
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${WD_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_RELEASE})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${WD_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_RELEASE})
	else()
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${WD_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_DEBUG})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${WD_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_${WD_DEV_BUILD_LINKERFLAGS}})
	endif()

	set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${WD_BUILDTYPENAME_RELEASE_UPPER} ${LINKER_FLAGS_RELEASE})

	if(WD_ENABLE_COMPILER_STATIC_ANALYSIS)
...
# #####################################
# ## wd_set_build_flags_clang(<target>)
# #####################################
function(wd_set_build_flags_clang TARGET_NAME)
	...
	if(WD_ENABLE_QT_SUPPORT)
		...
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${WD_QT_DIR}\"")
	endif()

	# Ignore any warnings caused by headers inside the ThirdParty directory.
	if(WD_SUBMODULE_PREFIX_PATH)
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CMAKE_SOURCE_DIR}/${WD_SUBMODULE_PREFIX_PATH}/Code/ThirdParty\"")
	else()
...
# #####################################
# ## wd_set_build_flags_gcc(<target>)
# #####################################
function(wd_set_build_flags_gcc TARGET_NAME)
...
# #####################################
# ## wd_set_build_flags(<target>)
# #####################################
function(wd_set_build_flags TARGET_NAME)
	...
	if(WD_CMAKE_COMPILER_MSVC)
		wd_set_build_flags_msvc(${TARGET_NAME} ${ARGN})
	endif()

	if(WD_CMAKE_COMPILER_CLANG)
		wd_set_build_flags_clang(${TARGET_NAME} ${ARGN})
	endif()

	if(WD_CMAKE_COMPILER_GCC)
		wd_set_build_flags_gcc(${TARGET_NAME} ${ARGN})
	endif()
endfunction()
