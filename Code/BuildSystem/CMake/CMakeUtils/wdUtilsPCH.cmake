...
# #####################################
# ## wd_retrieve_target_pch(<target> <pch-name>)
# #####################################
function(wd_retrieve_target_pch TARGET_NAME PCH_NAME)
...
# #####################################
# ## wd_pch_use(<pch-header> <cpp-files>)
# #####################################
function(wd_pch_use PCH_H TARGET_CPPS)
	if(NOT WD_USE_PCH OR WD_CMAKE_GENERATOR_MSVC)
...
# #####################################
# ## wd_pch_create(<pch-header> <cpp-file>)
# #####################################
function(wd_pch_create PCH_H TARGET_CPP)
	if(NOT WD_CMAKE_GENERATOR_MSVC)
		return()
	endif()
...
# #####################################
# ## wd_find_pch_in_file_list(<files> <out-pch-name>)
# #####################################
function(wd_find_pch_in_file_list FILE_LIST PCH_PATH PCH_NAME)
...
# #####################################
# ## wd_auto_pch(<target> <files> [<file-exclude-regex> ... ])
# #####################################
function(wd_auto_pch TARGET_NAME FILES)
	foreach(EXCLUDE_PATTERN ${ARGN})
		list(FILTER FILES EXCLUDE REGEX ${EXCLUDE_PATTERN})
	endforeach()

	wd_find_pch_in_file_list("${FILES}" PCH_PATH PCH_NAME)
	
	if(NOT PCH_NAME)
		return()
	endif()

	wd_pch_create("${PCH_NAME}.h" ${PCH_PATH})

	wd_pch_use("${PCH_NAME}.h" "${FILES}")

	wd_set_target_pch(${TARGET_NAME} ${PCH_NAME})
endfunction()
