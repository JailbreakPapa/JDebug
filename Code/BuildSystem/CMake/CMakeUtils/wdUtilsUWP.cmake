# #####################################
# ## wd_uwp_mark_import_as_content(<import>)
# #####################################

function(wd_uwp_mark_import_as_content IMPORT)
	if(NOT WD_CMAKE_PLATFORM_WINDOWS_UWP)
		return()
	endif()

	wd_pull_config_vars()

	get_target_property(_dll_location_debug ${IMPORT} IMPORTED_LOCATION_${WD_BUILDTYPENAME_DEBUG_UPPER})
	...
	if(${_dll_location_debug} STREQUAL ${_dll_location})
		...
		set_property(SOURCE ${_dll_location_debug} PROPERTY VS_DEPLOYMENT_CONTENT $<CONFIG:${WD_BUILDTYPENAME_DEBUG}>)

		set_property(SOURCE ${_dll_location} PROPERTY VS_DEPLOYMENT_CONTENT $<OR:$<CONFIG:${WD_BUILDTYPENAME_RELEASE}>,$<CONFIG:${WD_BUILDTYPENAME_DEV}>>)
...
# #####################################
# ## wd_uwp_add_import_to_sources(<target_name> <import>)
# #####################################
function(wd_uwp_add_import_to_sources TARGET_NAME IMPORT)
	if(NOT WD_CMAKE_PLATFORM_WINDOWS_UWP)
		return()
	endif()
	...
	get_target_property(_dll_location ${IMPORT} IMPORTED_LOCATION_${WD_BUILDTYPENAME_DEBUG_UPPER})
	target_sources(${TARGET_NAME} PUBLIC ${_dll_location})

	get_target_property(_dll_location ${IMPORT} IMPORTED_LOCATION_${WD_BUILDTYPENAME_DEV_UPPER})
	target_sources(${TARGET_NAME} PUBLIC ${_dll_location})

	get_target_property(_dll_location ${IMPORT} IMPORTED_LOCATION_${WD_BUILDTYPENAME_RELEASE_UPPER})
	target_sources(${TARGET_NAME} PUBLIC ${_dll_location})
...
# #####################################
# ## wd_uwp_add_default_content(<target>)
# #####################################
function(wd_uwp_fix_library_properties TARGET_NAME ALL_SOURCE_FILES)
	if(NOT WD_CMAKE_PLATFORM_WINDOWS_UWP)
		return()
	endif()
...
function(wd_uwp_add_default_content TARGET_NAME)
	wd_pull_all_vars()
	...
	set(CONTENT_DIRECTORY_SRC "${CMAKE_SOURCE_DIR}/${WD_SUBMODULE_PREFIX_PATH}/Data/Platform/UWP/")
