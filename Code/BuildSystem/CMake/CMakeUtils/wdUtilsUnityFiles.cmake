# #####################################
# ## wd_add_pvs_header(<target-var> <optional-text>)
# #####################################

function(wd_add_pvs_header TARGET_VAR OPTIONAL_TEXT)
	if(WD_ENABLE_PVS_STUDIO_HEADER_IN_UNITY_FILES)
...
# #####################################
# ## wd_generate_folder_unity_file(<target> <project-path> <rel-path-to-folder> <files-in-folder>)
# #####################################
function(wd_generate_folder_unity_file TARGET_NAME PROJECT_DIRECTORY SUB_FOLDER_PATH FILE_LIST)
	if(NOT ${WD_ENABLE_FOLDER_UNITY_FILES})
		return()
	endif()
	...
	# remove all "../"
	string(REGEX REPLACE "\\.\\.[\\/]" "" CURRENT_FOLDER_RELATIVE ${CURRENT_FOLDER_RELATIVE})
	...
	wd_add_pvs_header(UNITY_FILE_CONTENTS "// generated unity cpp file\n\n")

	# find the PCH and include it as the first thing in the unity file
	wd_retrieve_target_pch(${TARGET_NAME} PCH_FILE)

	if(PCH_FILE)
		string(APPEND UNITY_FILE_CONTENTS "#include <${PCH_FILE}.h>\n\n")

		wd_pch_use("${PCH_FILE}.h" "${FOLDER_UNITY_FILE}")
	endif()
...
# #####################################
# ## wd_generate_folder_unity_files_for_target(<target> <project-path> [<folder-to-exclude> ...])
# #####################################
function(wd_generate_folder_unity_files_for_target TARGET_NAME TARGET_DIRECTORY)
	wd_gather_subfolders(${TARGET_DIRECTORY} ALL_FOLDERS)
	...
	foreach(CUR_FOLDER ${ALL_FOLDERS})
		...
		wd_generate_folder_unity_file(${TARGET_NAME} "${TARGET_DIRECTORY}" "${CUR_FOLDER}" "${FILES_IN_FOLDER}")
	endforeach()
endfunction()
