...
///
/// All input handling is usually handled through the nsInputManager class.
...
///
/// By deriving from nsInputDevice you can extend what hardware the engine supports. The derived class should override
...
class NS_CORE_DLL nsInputDevice : public nsEnumerable<nsInputDevice, nsReflectedClass>
{
  NS_DECLARE_ENUMERABLE_CLASS_WITH_BASE(nsInputDevice, nsReflectedClass);
  NS_ADD_DYNAMIC_REFLECTION(nsInputDevice, nsReflectedClass);
...
public:
  ...
  float GetInputSlotState(nsStringView sSlot) const;
...
private:
  ...
  /// on Windows the WM_CHAR message sends this information. An nsInputDevice derived class should never try to compute this
  /// itself, but instead query this information from the OS, which will also handle localization.
  nsUInt32 RetrieveLastCharacter();

  /// \brief Calls UpdateHardwareState() on all devices.
  static void UpdateAllHardwareStates(nsTime tTimeDifference);
  ...
  static nsUInt32 RetrieveLastCharacterFromAllDevices();

protected:
  /// \brief Calls RegisterInputSlot() on the nsInputManager and passes the parameters through.
  static void RegisterInputSlot(nsStringView sName, nsStringView sDefaultDisplayName, nsBitflags<nsInputSlotFlags> SlotFlags); // [tested]
  ...
  nsMap<nsString, float> m_InputSlotValues; // [tested]

  /// \brief If this input device type handles character input, it should write the last typed character into this variable.
  /// The nsInputManager calls RetrieveLastCharacter() to query what the user typed last.
  nsUInt32 m_uiLastCharacter; // [tested]
  ...
  /// This will be the case when nsInputManager::PollHardware is used to make more frequent hardware updates
  ...
  /// \brief Override this, if you need to reset certain input slot values to zero, after the nsInputManager is finished with the current
  /// frame update.
  ...
  /// \brief This function is called once after nsInputManager::Update with the same time delta value.
  /// It allows to update hardware state, such as the vibration of gamepad motors.
  virtual void UpdateHardwareState(nsTime tTimeDifference) {}
