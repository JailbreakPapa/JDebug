...
/// \brief Data returned by nsResourceTypeLoader implementations.
struct NS_CORE_DLL nsResourceLoadData
{
...
class NS_CORE_DLL nsResourceTypeLoader
{
public:
  nsResourceTypeLoader() = default;
  virtual ~nsResourceTypeLoader() = default;
  ...
  ///
  /// \sa nsResourceLoadData
  virtual nsResourceLoadData OpenDataStream(const nsResource* pResource) = 0;
  ...
  virtual void CloseDataStream(const nsResource* pResource, const nsResourceLoadData& loaderData) = 0;

  /// \brief If this function returns true, a resource is unloaded and loaded again to update its content.
  ///
  /// Call nsResource::GetLoadedFileModificationTime() to query the file modification time that was returned
  /// through nsResourceLoadData::m_LoadedFileModificationDate.
  virtual bool IsResourceOutdated(const nsResource* pResource) const { return false; }
};

/// \brief A default implementation of nsResourceTypeLoader for standard file loading.
///
/// The loader will interpret the nsResource 'resource ID' as a path, read that full file into a memory stream.
...
class NS_CORE_DLL nsResourceLoaderFromFile : public nsResourceTypeLoader
{
public:
...
///
/// Use like this:
/// Allocate a nsResourceLoaderFromMemory instance on the heap, using NS_DEFAULT_NEW and store the result in a
/// nsUniquePtr<nsResourceTypeLoader>. Then set the description, the modification time (simply use nsTimestamp::CurrentTimestamp()), and the
/// custom data. Use a nsMemoryStreamWriter to write your custom data. Make sure to write EXACTLY the same format that the targeted resource
/// type would read, including all data that would typically be written by outside code, e.g. the default nsResourceLoaderFromFile
...
/// this yourself. Then call nsResourceManager::UpdateResourceWithCustomLoader(), specify the target resource and std::move your created
/// loader in there.
class NS_CORE_DLL nsResourceLoaderFromMemory : public nsResourceTypeLoader
{
public:
  virtual nsResourceLoadData OpenDataStream(const nsResource* pResource) override;
  virtual void CloseDataStream(const nsResource* pResource, const nsResourceLoadData& loaderData) override;
  ...
  nsString m_sResourceDescription;
  nsTimestamp m_ModificationTimestamp;
  nsDefaultMemoryStreamStorage m_CustomData;
