...
bool nsLuaWrapper::IsVariableAvailable(const char* szName) const
{
...
bool nsLuaWrapper::IsFunctionAvailable(const char* szFunction) const
{
  if (m_States.m_iOpenTables == 0)
...
nsInt32 nsLuaWrapper::GetIntVariable(const char* szName, nsInt32 iDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
bool nsLuaWrapper::GetBoolVariable(const char* szName, bool bDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
float nsLuaWrapper::GetFloatVariable(const char* szName, float fDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
const char* nsLuaWrapper::GetStringVariable(const char* szName, const char* szDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
