...
/// \brief The nsFileSystem provides high-level functionality to manage files in a virtual file system.
///
...
/// Different nsDataDirectoryType's can implement these different 'decoding methods', i.e. they handle how to actually
...
/// The default nsFileReader and nsFileWriter implement a buffering policy, i.e. they use an internal cache to only
...
/// reader/writer class.
/// How to mount data directories (i.e. with which nsDataDirectoryType) is defined by the 'DataDirFactories', which
/// are functions that create nsDataDirectoryType's. This way one can mount the same data directory (e.g. "MyTestDir")
...
///
/// Additionally nsFileSystem also broadcasts events about which files are (about to be) accessed.
...
///
/// All operations that go through the nsFileSystem are protected by a mutex, which means that opening, closing, deleting
...
class NS_FOUNDATION_DLL nsFileSystem
{
public:
  ...
  static nsEventSubscriptionID RegisterEventHandler(nsEvent<const FileEvent&>::Handler handler);

  /// \brief Unregisters a previously registered Event Handler.
  static void UnregisterEventHandler(nsEvent<const FileEvent&>::Handler handler);
  static void UnregisterEventHandler(nsEventSubscriptionID subscriptionId);
...
public:
  ...
  /// on a single thread with no other thread accessing anything in nsFileSystem simultaneously.
  ///@{
  ...
  /// can successfully create an nsDataDirectoryType. In this process the last factory added has the highest priority.
  /// Once a factory is found that was able to create a nsDataDirectoryType, that one is used.
  ...
  using nsDataDirFactory = nsDataDirectoryType* (*)(nsStringView, nsStringView, nsStringView, nsFileSystem::DataDirUsage);
  ...
  static void RegisterDataDirectoryFactory(nsDataDirFactory factory, float fPriority = 0); // [tested]
  ...
  static nsResult AddDataDirectory(nsStringView sDataDirectory, nsStringView sGroup = {}, nsStringView sRootName = {}, nsFileSystem::DataDirUsage usage = ReadOnly); // [tested]
  ...
  static bool RemoveDataDirectory(nsStringView sRootName);
  ...
  static nsUInt32 RemoveDataDirectoryGroup(nsStringView sGroup); // [tested]
  ...
  static const DataDirectoryInfo* FindDataDirectoryWithRoot(nsStringView sRootName);

  /// \brief Returns the number of currently active data directories.
  static nsUInt32 GetNumDataDirectories(); // [tested]
  ...
  ///
  /// Additionally the 'redirection file' feature of nsFileSystem::FindFolderWithSubPath() is used
  /// to allow finding a relocated SDK folder. To do that, place a file called 'nsSdkRoot.txt'
  ...
  ///
  /// \sa nsFileSystem::FindFolderWithSubPath()
  static nsResult DetectSdkRootDirectory(nsStringView sExpectedSubFolder = "Data/Base");

  /// \brief the special directory ">Sdk" is the root folder of the SDK data, it is often used as the main reference
  /// from where other data directories are found. For higher level code (e.g. nsApplication) it is often vital that this is set early at startup.
  ///
  /// \sa DetectSdkRootDirectory()
  static void SetSdkRootDirectory(nsStringView sSdkDir);
  ...
  /// \sa DetectSdkRootDirectory
  static nsStringView GetSdkRootDirectory();
  ...
  static void SetSpecialDirectory(nsStringView sName, nsStringView sReplacement);
  ...
  /// ">user/" - Resolves to what nsOSFile::GetUserDataFolder() returns.
  /// ">temp/" - Resolves to what nsOSFile::GetTempDataFolder() returns.
  /// ">appdir/" - Resolves to what nsOSFile::GetApplicationDirectory() returns.
  ///
  /// Returns NS_FAILURE if \a szDirectory starts with an unknown special directory.
  static nsResult ResolveSpecialDirectory(nsStringView sDirectory, nsStringBuilder& out_sPath);
  ...
  static void StartSearch(nsFileSystemIterator& ref_iterator, nsStringView sSearchTerm, nsBitflags<nsFileSystemIteratorFlags> flags = nsFileSystemIteratorFlags::Default);
#endif

  ///@}

  static nsResult CreateDirectoryStructure(nsStringView sPath);
...
public:
  ...
  static void DeleteFile(nsStringView sFile); // [tested]
  ...
  /// \brief Tries to get the nsFileStats for the given file.
  /// Typically should give the same results as nsOSFile::GetFileStats, but some data dir implementations may not support
  /// retrieving all data (e.g. GetFileStats on folders might not always work).
  static nsResult GetFileStats(nsStringView sFileOrFolder, nsFileStats& out_stats);
  ...
  static nsResult ResolvePath(nsStringView sPath, nsStringBuilder* out_pAbsolutePath, nsStringBuilder* out_pDataDirRelativePath, nsDataDirectoryType** out_pDataDir = nullptr); // [tested]

  /// \brief Starts at szStartDirectory and goes up until it finds a folder that contains the given sub folder structure.
  ///
  /// Returns NS_FAILURE if nothing is found. Otherwise \a result is the absolute path to the existing folder that has a given sub-folder.
  ///
  ...
  static bool ResolveAssetRedirection(nsStringView sPathOrAssetGuid, nsStringBuilder& out_sRedirection);
  ...
  static nsStringView MigrateFileLocation(nsStringView sOldLocation, nsStringView sNewLocation);

private:
  friend class nsDataDirectoryReaderWriterBase;
  friend class nsFileReaderBase;
  friend class nsFileWriterBase;

  /// \brief This is used by the actual file readers (like nsFileReader) to get an abstract file reader.
  ///
  ...
  static nsDataDirectoryReader* GetFileReader(nsStringView sFile, nsFileShareMode::Enum FileShareMode, bool bAllowFileEvents);

  /// \brief This is used by the actual file writers (like nsFileWriter) to get an abstract file writer.
  ///
  ...
  /// This should usually be set to true, unless code is already acting on a file event and needs to do a file operation
  /// itself, which should not trigger an endless recursion of file events.
  static nsDataDirectoryWriter* GetFileWriter(nsStringView sFile, nsFileShareMode::Enum FileShareMode, bool bAllowFileEvents);


private:
  NS_MAKE_SUBSYSTEM_STARTUP_FRIEND(Foundation, FileSystem);
...
private:
  ...
  static nsString s_sSdkRootDir;
  static nsMap<nsString, nsString> s_SpecialDirectories;
  static FileSystemData* s_pData;
};

/// \brief Describes the type of events that are broadcast by the nsFileSystem.
struct nsFileSystem::FileEventType
{
  enum Enum
  {
...
/// \brief The event data that is broadcast by the nsFileSystem upon certain file operations.
struct nsFileSystem::FileEvent
{
  /// \brief The exact event that occurred.
  nsFileSystem::FileEventType::Enum m_EventType = FileEventType::None;

  /// \brief Path to the file or directory that was involved.
  nsStringView m_sFileOrDirectory;

  /// \brief Additional Path / Name that might be of interest.
  nsStringView m_sOther;

  /// \brief The data-directory, that was involved.
  const nsDataDirectoryType* m_pDataDir = nullptr;
};
