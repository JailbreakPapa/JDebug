...
/// \brief The default class to use to write data to a file, implements the wdStreamWriter interface.
///
...
class WD_FOUNDATION_DLL wdFileWriter : public wdFileWriterBase
{
  WD_DISALLOW_COPY_AND_ASSIGN(wdFileWriter);

public:
  /// \brief Constructor, does nothing.
  wdFileWriter() = default;

  /// \brief Destructor, closes the file, if it is still open (RAII).
  ~wdFileWriter() { Close(); }

  /// \brief Opens the given file for writing. Returns WD_SUCCESS if the file could be opened. A cache is created to speed up small writes.
  ///
  ...
  wdResult Open(wdStringView sFile, wdUInt32 uiCacheSize = 1024 * 1024, wdFileShareMode::Enum fileShareMode = wdFileShareMode::Default,
    bool bAllowFileEvents = true);

  /// \brief Closes the file, if it is open.
  void Close();

  /// \brief Writes the given number of bytes to the file. Returns WD_SUCCESS if all bytes were successfully written.
  ///
  /// As this class buffers writes with an internal cache, WD_SUCCESS does NOT mean that the data is actually written to disk.
  ...
  virtual wdResult Flush() override;

private:
  wdUInt64 m_uiCacheWritePosition;
