...
public:
  ...
  wdBoundingBoxTemplate(const wdVec3Template<Type>& vMin, const wdVec3Template<Type>& vMax); // [tested]
  ...
  void SetCenterAndHalfExtents(const wdVec3Template<Type>& vCenter, const wdVec3Template<Type>& vHalfExtents); // [tested]
  ...
  void SetElements(const wdVec3Template<Type>& vMin, const wdVec3Template<Type>& vMax); // [tested]
  ...
  void GetCorners(wdVec3Template<Type>* out_pCorners) const; // [tested]

  /// \brief Returns the center position of the box.
  const wdVec3Template<Type> GetCenter() const; // [tested]

  /// \brief Returns the extents of the box along each axis.
  const wdVec3Template<Type> GetExtents() const; // [tested]

  /// \brief Returns the half extents of the box along each axis.
  const wdVec3Template<Type> GetHalfExtents() const; // [tested]
  ...
  void ExpandToInclude(const wdBoundingBoxTemplate& rhs); // [tested]
  ...
  void Grow(const wdVec3Template<Type>& vDiff); // [tested]
  ...
  bool Contains(const wdBoundingBoxTemplate& rhs) const; // [tested]
  ...
  bool Contains(const wdBoundingSphereTemplate<Type>& sphere) const; // [tested]

  /// \brief Checks whether this box overlaps with the given box.
  bool Overlaps(const wdBoundingBoxTemplate& rhs) const; // [tested]
  ...
  bool Overlaps(const wdBoundingSphereTemplate<Type>& sphere) const; // [tested]

  /// \brief Checks whether this box and the other box are exactly identical.
  bool IsIdentical(const wdBoundingBoxTemplate& rhs) const; // [tested]

  /// \brief Checks whether this box and the other box are equal within some threshold.
  bool IsEqual(const wdBoundingBoxTemplate& rhs, Type fEpsilon = wdMath::DefaultEpsilon<Type>()) const; // [tested]

  /// \brief Moves the box by the given vector.
  void Translate(const wdVec3Template<Type>& vDiff); // [tested]

  /// \brief Scales the box along each axis, but keeps its center constant.
  void ScaleFromCenter(const wdVec3Template<Type>& vScale); // [tested]
  ...
  const wdVec3Template<Type> GetClampedPoint(const wdVec3Template<Type>& vPoint) const; // [tested]

  /// \brief Returns the squared minimum distance from the box's surface to the point. Zero if the point is inside the box.
  Type GetDistanceSquaredTo(const wdVec3Template<Type>& vPoint) const; // [tested]

  /// \brief Returns the minimum squared distance between the two boxes. Zero if the boxes overlap.
  Type GetDistanceSquaredTo(const wdBoundingBoxTemplate& rhs) const; // [tested]
  ...
  Type GetDistanceTo(const wdBoundingSphereTemplate<Type>& sphere) const; // [tested]

  /// \brief Returns the minimum distance between the two boxes. Zero if the boxes overlap.
  Type GetDistanceTo(const wdBoundingBoxTemplate& rhs) const; // [tested]
  ...
  bool GetRayIntersection(const wdVec3Template<Type>& vStartPos, const wdVec3Template<Type>& vRayDir, Type* out_pIntersectionDistance = nullptr,
  ...
  bool GetLineSegmentIntersection(const wdVec3Template<Type>& vStartPos, const wdVec3Template<Type>& vEndPos, Type* out_pLineFraction = nullptr,
    wdVec3Template<Type>* out_pIntersection = nullptr) const; // [tested]

  /// \brief Returns a bounding sphere that encloses this box.
  const wdBoundingSphereTemplate<Type> GetBoundingSphere() const; // [tested]


public:
  wdVec3Template<Type> m_vMin;
  wdVec3Template<Type> m_vMax;
};

/// \brief Checks whether this box and the other are identical.
template <typename Type>
bool operator==(const wdBoundingBoxTemplate<Type>& lhs, const wdBoundingBoxTemplate<Type>& rhs); // [tested]
