...
const nsTimestamp nsDateTime::GetTimestamp() const
{
  ...
  if (timeinfo.tm_isdst == 1)
    iTimeStamp -= 3600;
  return nsTimestamp::MakeFromInt(iTimeStamp, nsSIUnitOfTime::Second);
}

nsResult nsDateTime::SetFromTimestamp(nsTimestamp timestamp)
{
  tm timeinfo = {0};
  time64_t iTime = (time64_t)timestamp.GetInt64(nsSIUnitOfTime::Second);
  ...
  m_uiDayOfWeek = nsMath::MaxValue<nsUInt8>(); // TODO: no day of week exists, setting to uint8 max.
  m_uiMicroseconds = 0;
