...
void nsSystemInformation::Initialize()
{
  ...
  s_SystemInformation.m_uiCPUCoreCount = static_cast<nsUInt32>(sysconf(_SC_NPROCESSORS_ONLN));

  nsUInt64 uiPageSize = sysconf(_SC_PAGE_SIZE);

  s_SystemInformation.m_uiMemoryPageSize = static_cast<nsUInt32>(uiPageSize);
...
nsUInt64 nsSystemInformation::GetAvailableMainMemory() const
{
  ...
  return static_cast<nsUInt64>(s_SystemInformation.m_uiMemoryPageSize) * static_cast<nsUInt64>(vmt.t_free);
}
