...
namespace
{
  ...
  struct StackTracerImplementation
  {
    ...
    {
      nsMemoryUtils::ZeroFill(this, 1);

      kernel32Dll = LoadLibraryW(L"kernel32.dll");
      NS_ASSERT_DEV(kernel32Dll != nullptr, "StackTracer could not load kernel32.dll");
      ...
      NS_ASSERT_DEV(dbgHelpDll != nullptr, "StackTracer could not load dbghelp.dll");
  ...
  static void Initialize()
  {
    ...
    {
      alignas(NS_ALIGNMENT_OF(StackTracerImplementation)) static nsUInt8 ImplementationBuffer[sizeof(StackTracerImplementation)];
      s_pImplementation = new (ImplementationBuffer) StackTracerImplementation();
      NS_ASSERT_DEV(s_pImplementation != nullptr, "StackTracer initialization failed");
    }
  }
  ...
  static bool SymbolInitializeImpl()
  {
    ...
    // so when using NS together with for example a third party library that also records stack traces, just calling SymInitialize will fail
  ...
  static void SymbolInitialize()
  {
    ...
    {
      nsLog::Error("StackTracer could not initialize symbols. Error-Code {0}", nsArgErrorCode(::GetLastError()));
      return;
    }
    ...
    if (!(*s_pImplementation->symSetSearchPath)(GetCurrentProcess(), nsStringWChar(nsOSFile::GetApplicationDirectory())))
    {
      nsLog::Error("StackTracer could not set symbol search path. Error-Code {0}", nsArgErrorCode(::GetLastError()));
      return;
    }
  }
} // namespace
...
void nsStackTracer::OnPluginEvent(const nsPluginEvent& e)
{
  ...
  if (e.m_EventType == nsPluginEvent::BeforeUnloading)
  {
    SymbolInitialize();
  }

  if (false) // e.m_EventType == nsPluginEvent::AfterLoading)
  {
    nsStringBuilder tmp;

    char buffer[1024];
    strcpy_s(buffer, nsOSFile::GetApplicationDirectory().GetStartPointer());
    strcat_s(buffer, e.m_sPluginBinary.GetData(tmp));
    strcat_s(buffer, ".dll");

    wchar_t szPluginPath[1024];
    mbstowcs(szPluginPath, buffer, NS_ARRAY_SIZE(szPluginPath));

    wchar_t szPluginName[256];
    mbstowcs(szPluginName, e.m_sPluginBinary.GetData(tmp), NS_ARRAY_SIZE(szPluginName));
    ...
    if (moduleAddress == 0)
    {
      ...
      {
        nsLog::Error("StackTracer could not load symbols for '{0}'. Error-Code {1}", e.m_sPluginBinary, nsArgErrorCode(err));
      }
    ...
    nsMemoryUtils::ZeroFill(&moduleInfo, 1);
    ...
    if (!(*s_pImplementation->getModuleInfo)(currentProcess, moduleAddress, &moduleInfo))
    {
      ...
      char errStr[1024];
      nsStringUtils::snprintf(errStr, 1024, "StackTracer could not get module info for '%s'. Error-Code %u (\"%s\")\n", e.m_sPluginBinary.GetData(tmp), err, static_cast<char*>(lpMsgBuf));
      nsLog::Print(errStr);
...
// static
nsUInt32 nsStackTracer::GetStackTrace(nsArrayPtr<void*>& ref_trace, void* pContext)
{
  Initialize();
...
#  endif
    ...
    for (nsInt32 i = 0; i < (nsInt32)ref_trace.GetCount(); i++)
    {
      ...
      else
      {
        ...
        return nsMath::Max(i - 4, 0);
      }
    }
  }
  else if (s_pImplementation->captureStackBackTrace != nullptr)
  {
    const nsUInt32 uiSkip = 1;
    const nsUInt32 uiMaxNumTrace = nsMath::Min(62U, ref_trace.GetCount());
    nsInt32 iNumTraces = (*s_pImplementation->captureStackBackTrace)(uiSkip, uiMaxNumTrace, ref_trace.GetPtr(), nullptr);

    // skip the last three stack-frames since they are useless
    return nsMath::Max(iNumTraces - 3, 0);
  }
...
void nsStackTracer::ResolveStackTrace(const nsArrayPtr<void*>& trace, PrintFunc printFunc)
{
  ...
  if (s_pImplementation->symbolFromAddress != nullptr && s_pImplementation->lineFromAdress != nullptr)
  {
    ...
    for (nsUInt32 i = 0; i < uiNumTraceEntries; i++)
    {
      ...
      char finalStr[1024];
      wcstombs(finalStr, str, NS_ARRAY_SIZE(finalStr));
