...
class WD_FOUNDATION_DLL wdReflectionSerializer
{
public:
  ...
  static void WriteObjectToDDL(wdStreamWriter& inout_stream, const wdRTTI* pRtti, const void* pObject, bool bCompactMmode = true,
    wdOpenDdlWriter::TypeStringMode typeMode = wdOpenDdlWriter::TypeStringMode::Shortest); // [tested]

  /// \brief Overload of WriteObjectToDDL that takes an existing DDL writer to output to.
  static void WriteObjectToDDL(wdOpenDdlWriter& ref_ddl, const wdRTTI* pRtti, const void* pObject, wdUuid guid = wdUuid()); // [tested]

  /// \brief Same as WriteObjectToDDL but binary.
  static void WriteObjectToBinary(wdStreamWriter& inout_stream, const wdRTTI* pRtti, const void* pObject); // [tested]
  ...
  static void* ReadObjectFromDDL(wdStreamReader& inout_stream, const wdRTTI*& ref_pRtti); // [tested]

  static void* ReadObjectFromDDL(const wdOpenDdlReaderElement* pRootElement, const wdRTTI*& ref_pRtti); // [tested]

  /// \brief Same as ReadObjectFromDDL but binary.
  static void* ReadObjectFromBinary(wdStreamReader& inout_stream, const wdRTTI*& ref_pRtti); // [tested]
  ...
  static void ReadObjectPropertiesFromDDL(wdStreamReader& inout_stream, const wdRTTI& rtti, void* pObject); // [tested]

  /// \brief Same as ReadObjectPropertiesFromDDL but binary.
  static void ReadObjectPropertiesFromBinary(wdStreamReader& inout_stream, const wdRTTI& rtti, void* pObject); // [tested]
  ...
  static void* Clone(const void* pObject, const wdRTTI* pType); // [tested]
  ...
  static void Clone(const void* pObject, void* pClone, const wdRTTI* pType); // [tested]

  /// \brief Templated convenience function that calls Clone and automatically deduces the type.
  template <typename T>
  static T* Clone(const T* pObject)
  {
    return static_cast<T*>(Clone(pObject, wdGetStaticRTTI<T>()));
  }
};
