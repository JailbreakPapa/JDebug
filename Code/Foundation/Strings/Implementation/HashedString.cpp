...
void nsHashedString::InitHashedString()
{
  ...
  alignas(NS_ALIGNMENT_OF(HashedStringData)) static nsUInt8 HashedStringDataBuffer[sizeof(HashedStringData)];
...
{
  NS_CHECK_AT_COMPILETIME_MSG(sizeof(m_Data) == sizeof(void*), "The hashed string data should only be as large as one pointer.");
  NS_CHECK_AT_COMPILETIME_MSG(sizeof(*this) == sizeof(void*), "The hashed string data should only be as large as one pointer.");
