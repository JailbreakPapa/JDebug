...
void wdSystemInformation::Initialize()
{
        ...
        HStringReference(RuntimeClass_Windows_Networking_Connectivity_NetworkInformation).Get(), &networkInformation)))
  {
    ...
    {
      wdUwpUtils::wdWinRtIterateIVectorView<IHostName*>(hostNames, [](UINT, IHostName* hostName) {
        ...
        if (hostNameType == HostNameType_DomainName)
        {
          ...
          wdStringUtils::Copy(s_SystemInformation.m_sHostName, sizeof(s_SystemInformation.m_sHostName), wdStringUtf8(name).GetData());
          return false;
        }
...
float wdSystemInformation::GetCPUUtilization() const
{
  ...
  return wdMath::Clamp(util, 0.f, 1.f) * 100.f;
