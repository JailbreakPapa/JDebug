#include <Foundation/FoundationInternal.h>
WD_FOUNDATION_INTERNAL_HEADER

#include <errno.h>
#include <pthread.h>

wdConditionVariable::wdConditionVariable()
{
  pthread_cond_init(&m_Data.m_ConditionVariable, nullptr);
}

wdConditionVariable::~wdConditionVariable()
{
  WD_ASSERT_DEV(m_iLockCount == 0, "Thread-signal must be unlocked during destruction.");

  pthread_cond_destroy(&m_Data.m_ConditionVariable);
}

void wdConditionVariable::SignalOne()
{
  pthread_cond_signal(&m_Data.m_ConditionVariable);
}

void wdConditionVariable::SignalAll()
{
  pthread_cond_broadcast(&m_Data.m_ConditionVariable);
}

void wdConditionVariable::UnlockWaitForSignalAndLock() const
{
  WD_ASSERT_DEV(m_iLockCount > 0, "wdConditionVariable must be locked when calling UnlockWaitForSignalAndLock.");

  pthread_cond_wait(&m_Data.m_ConditionVariable, &m_Mutex.GetMutexHandle());
}

wdConditionVariable::WaitResult wdConditionVariable::UnlockWaitForSignalAndLock(wdTime timeout) const
{
  WD_ASSERT_DEV(m_iLockCount > 0, "wdConditionVariable must be locked when calling UnlockWaitForSignalAndLock.");

  // inside the lock
  --m_iLockCount;

  timeval now;
  gettimeofday(&now, nullptr);

  // pthread_cond_timedwait needs an absolute time value, so compute it from the current time.
  struct timespec timeToWait;

  const wdInt64 iNanoSecondsPerSecond = 1000000000LL;
  const wdInt64 iMicroSecondsPerNanoSecond = 1000LL;

  wdInt64 endTime = now.tv_sec * iNanoSecondsPerSecond + now.tv_usec * iMicroSecondsPerNanoSecond + static_cast<wdInt64>(timeout.GetNanoseconds());

  timeToWait.tv_sec = endTime / iNanoSecondsPerSecond;
  timeToWait.tv_nsec = endTime % iNanoSecondsPerSecond;

  if (pthread_cond_timedwait(&m_Data.m_ConditionVariable, &m_Mutex.GetMutexHandle(), &timeToWait) == ETIMEDOUT)
  {
    // inside the lock
    ++m_iLockCount;
    return WaitResult::Timeout;
  }

  // inside the lock
  ++m_iLockCount;
  return WaitResult::Signaled;
}
