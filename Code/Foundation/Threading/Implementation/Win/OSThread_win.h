...
#define WD_MSVC_WARNING_NUMBER 6312
...
void SetThreadName(HANDLE hThread, LPCSTR pThreadName)
{
  ...
  {
    wdStringWChar threadName(pThreadName);
...
wdOSThread::wdOSThread(
  wdOSThreadEntryPoint threadEntryPoint, void* pUserData /*= nullptr*/, const char* szName /*= "wdThread"*/, wdUInt32 uiStackSize /*= 128 * 1024*/)
{
  s_iThreadCount.Increment();

  WD_ASSERT_ALWAYS(threadEntryPoint != nullptr, "Thread entry point is invalid.");

  m_hHandle = CreateThread(nullptr, uiStackSize, threadEntryPoint, pUserData, CREATE_SUSPENDED, nullptr);
  WD_ASSERT_RELEASE(m_hHandle != INVALID_HANDLE_VALUE, "Thread creation failed!");
  WD_ASSERT_RELEASE(m_hHandle != nullptr, "Thread creation failed!"); // makes the static code analysis happy
