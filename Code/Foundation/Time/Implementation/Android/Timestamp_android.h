...
const wdTimestamp wdDateTime::GetTimestamp() const
{
  ...
  if (timeinfo.tm_isdst == 1)
    iTimeStamp -= 3600;
  return wdTimestamp(iTimeStamp, wdSIUnitOfTime::Second);
}

bool wdDateTime::SetTimestamp(wdTimestamp timestamp)
{
  tm timeinfo = {0};
  time64_t iTime = (time64_t)timestamp.GetInt64(wdSIUnitOfTime::Second);
  ...
  m_uiDayOfWeek = wdMath::MaxValue<wdUInt8>(); // TODO: no day of week exists, setting to uint8 max.
  m_uiMicroseconds = 0;
