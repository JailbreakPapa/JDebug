...
/// or GetFloatOption()
class NS_FOUNDATION_DLL nsCommandLineUtils
{
public:
  enum ArgMode
  {
  ...
  /// \brief Returns one global instance of nsCommandLineUtils.
  static nsCommandLineUtils* GetGlobalInstance();
  ...
  static void SplitCommandLineString(const char* szCommandString, bool bAddExecutableDir, nsDynamicArray<nsString>& out_args, nsDynamicArray<const char*>& out_argsV);

  /// \brief Initializes nsCommandLineUtils from the parameter arguments that were passed to the application.
  void SetCommandLine(nsUInt32 uiArgc, const char** pArgv, ArgMode mode = UseArgcArgv); // [tested]

  /// \brief Overload for non-const argv versions.
  void SetCommandLine(nsUInt32 uiArgc, char** pArgv, ArgMode mode = UseArgcArgv)
  {
    SetCommandLine(uiArgc, const_cast<const char**>(pArgv), mode);
  }

  /// \brief Initializes nsCommandLineUtils from a list of already split up commands.
  void SetCommandLine(nsArrayPtr<nsString> commands);

#if NS_ENABLED(NS_PLATFORM_WINDOWS_DESKTOP)
  /// \brief Initializes nsCommandLineUtils by querying the command line parameters directly from the OS.
  ///
  ...
  const nsDynamicArray<nsString>& GetCommandLineArray() const;

  /// \brief Assembles the original command line from the split up string representation.
  nsString GetCommandLineString() const;
  ...
  nsUInt32 GetParameterCount() const; // [tested]

  /// \brief Returns the n-th parameter string that was passed to the application.
  const nsString& GetParameter(nsUInt32 uiParam) const; // [tested]
  ...
  nsInt32 GetOptionIndex(nsStringView sOption, bool bCaseSensitive = false) const; // [tested]

  /// \brief Returns whether the requested option is specified, at all.
  bool HasOption(nsStringView sOption, bool bCaseSensitive = false) const;
  ...
  nsUInt32 GetStringOptionArguments(nsStringView sOption, bool bCaseSensitive = false) const; // [tested]
  ...
  nsStringView GetStringOption(nsStringView sOption, nsUInt32 uiArgument = 0, nsStringView sDefault = {},
  ...
  const nsString GetAbsolutePathOption(nsStringView sOption, nsUInt32 uiArgument = 0, nsStringView sDefault = {}, bool bCaseSensitive = false) const;
  ...
  ///   If there is one parameter following, it is interpreted using nsConversionUtils::StringToBool().
  ///   If that conversion fails, bDefault is returned.
  bool GetBoolOption(nsStringView sOption, bool bDefault = false, bool bCaseSensitive = false) const; // [tested]
  ...
  ///   it is interpreted using nsConversionUtils::StringToInt().
  ///   If that conversion fails or there is no such option or no parameter follows it, iDefault is returned.
  nsInt32 GetIntOption(nsStringView sOption, nsInt32 iDefault = 0, bool bCaseSensitive = false) const; // [tested]

  /// \brief Same as GetIntOption() but assumes the value is a uint32.
  nsUInt32 GetUIntOption(nsStringView sOption, nsUInt32 uiDefault = 0, bool bCaseSensitive = false) const; // [tested]
  ...
  /// \return
  ///   If an option with the name \a szOption can be found, and there is one parameter following,
  ///   it is interpreted using nsConversionUtils::StringToFloat().
  ///   If that conversion fails or there is no such option or no parameter follows it, fDefault is returned.
  double GetFloatOption(nsStringView sOption, double fDefault = 0.0, bool bCaseSensitive = false) const; // [tested]
  ...
  void InjectCustomArgument(nsStringView sArgument); // [tested]

private:
  nsDynamicArray<nsString> m_Commands;
};
