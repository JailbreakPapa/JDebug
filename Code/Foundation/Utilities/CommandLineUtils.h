...
/// or GetFloatOption()
class WD_FOUNDATION_DLL wdCommandLineUtils
{
public:
  enum ArgMode
  {
  ...
  /// \brief Returns one global instance of wdCommandLineUtils.
  static wdCommandLineUtils* GetGlobalInstance();
  ...
  static void SplitCommandLineString(const char* szCommandString, bool bAddExecutableDir, wdDynamicArray<wdString>& out_args, wdDynamicArray<const char*>& out_argsV);

  /// \brief Initializes wdCommandLineUtils from the parameter arguments that were passed to the application.
  void SetCommandLine(wdUInt32 uiArgc, const char** pArgv, ArgMode mode = UseArgcArgv); // [tested]

  /// \brief Initializes wdCommandLineUtils from a list of already split up commands.
  void SetCommandLine(wdArrayPtr<wdString> commands);

#if WD_ENABLED(WD_PLATFORM_WINDOWS_DESKTOP)
  /// \brief Initializes wdCommandLineUtils by querying the command line parameters directly from the OS.
  ///
  ...
  const wdDynamicArray<wdString>& GetCommandLineArray() const;

  /// \brief Assembles the original command line from the split up string representation.
  wdString GetCommandLineString() const;
  ...
  wdUInt32 GetParameterCount() const; // [tested]

  /// \brief Returns the n-th parameter string that was passed to the application.
  const char* GetParameter(wdUInt32 uiParam) const; // [tested]
  ...
  wdInt32 GetOptionIndex(const char* szOption, bool bCaseSensitive = false) const; // [tested]
  ...
  wdUInt32 GetStringOptionArguments(const char* szOption, bool bCaseSensitive = false) const; // [tested]
  ...
  const char* GetStringOption(const char* szOption, wdUInt32 uiArgument = 0, const char* szDefault = "",
  ...
  const wdString GetAbsolutePathOption(const char* szOption, wdUInt32 uiArgument = 0, const char* szDefault = "", bool bCaseSensitive = false) const;
  ...
  ///   If there is one parameter following, it is interpreted using wdConversionUtils::StringToBool().
  ...
  ///   it is interpreted using wdConversionUtils::StringToInt().
  ///   If that conversion fails or there is no such option or no parameter follows it, iDefault is returned.
  wdInt32 GetIntOption(const char* szOption, wdInt32 iDefault = 0, bool bCaseSensitive = false) const; // [tested]

  /// \brief Same as GetIntOption() but assumes the value is a uint32.
  wdUInt32 GetUIntOption(const char* szOption, wdUInt32 uiDefault = 0, bool bCaseSensitive = false) const; // [tested]
  ...
  /// \return
  ///   If an option with the name \a szOption can be found, and there is one parameter following,
  ///   it is interpreted using wdConversionUtils::StringToFloat().
...
private:
  wdDynamicArray<wdString> m_Commands;
};
