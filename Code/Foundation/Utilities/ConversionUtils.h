...
/// To convert from numerical values to strings, use wdStringBuilder::Format, which provides a rich set of formatting options.
namespace wdConversionUtils
{
  /// \brief Parses szString and converts it to an integer value. Returns WD_FAILURE if the string contains no parsable integer value.
  ///
  /// \param szString
  ///   If szString is nullptr or an empty string or starts with an some non-whitespace and non-sign character, WD_FAILURE is returned.
  ...
  ///   position where the parser stopped (returning WD_SUCCESS).
  ...
  /// \return
  ///   WD_SUCCESS if any integer value could get properly extracted from szString (including 0). This includes that only some part of the
  ///   string was parsed until a non-digit character was encountered.
  ///   WD_FAILURE if the string starts with something that can not be interpreted as an integer.
  WD_FOUNDATION_DLL wdResult StringToInt(wdStringView sText, wdInt32& out_iRes, const char** out_pLastParsePosition = nullptr); // [tested]

  /// \brief Same as StringToInt() but expects the string to be a uint32.
  ///
  /// If the parsed value is a valid int but outside the uint32 value range, the function returns WD_FAILURE.
  WD_FOUNDATION_DLL wdResult StringToUInt(wdStringView sText, wdUInt32& out_uiRes, const char** out_pLastParsePosition = nullptr); // [tested]

  /// \brief Same as StringToInt but converts to a 64bit integer value instead.
  WD_FOUNDATION_DLL wdResult StringToInt64(wdStringView sText, wdInt64& out_iRes,
    const char** out_pLastParsePosition = nullptr); // [tested]

  /// \brief Parses szString and converts it to a double value. Returns WD_FAILURE if the string contains no parseable floating point value.
  ///
  /// \param szString
  ///   If szString is nullptr or an empty string or starts with some non-whitespace and non-sign character, WD_FAILURE is returned.
  ///   All whitespace at the start of the string are skipped. Each minus sign flips the sign of the output value, so --3 will return 3.
  ///   Plus signs are skipped and have no effect, so -+3 will still return -3.
  ...
  /// \param out_Res
  ///   If WD_SUCCESS is returned, out_Res will contain the result. Otherwise it stays unmodified.
  ...
  /// \return
  ///   WD_SUCCESS if any text was encountered that can be interpreted as a floating point value.
  ///   WD_FAILURE otherwise.
  ///
  /// \note
  ...
  WD_FOUNDATION_DLL wdResult StringToFloat(wdStringView sText, double& out_fRes, const char** out_pLastParsePosition = nullptr); // [tested]
  ...
  ///
  /// \param szString
  ///   If szString starts with whitespace characters, they are skipped. WD_SUCCESS is returned (and out_Res is filled with true/false),
  ///   if the string then starts with any of the following phrases: "true", "false", "on", "off", "yes", "no", "1", "0", "enable",
  ///   "disable". WD_FAILURE is returned if none of those is encountered (or the string is empty). It does not matter, whether the string
  ...
  /// \param out_Res
  ///   If WD_SUCCESS is returned, out_Res contains a valid value. Otherwise it is not modified. That means you can initialize it with a
  ///   default value that can be used even if WD_FAILURE is returned.
  /// \param out_LastParsePosition
  ///   On success out_LastParsePosition will contain the address of the character in szString that stopped the parser. This might point
  ...
  /// \return
  ///   WD_SUCCESS if any phrase was encountered that can be interpreted as a boolean value.
  ///   WD_FAILURE otherwise.
  WD_FOUNDATION_DLL wdResult StringToBool(wdStringView sText, bool& out_bRes, const char** out_pLastParsePosition = nullptr); // [tested]
  ...
  WD_FOUNDATION_DLL wdUInt32 ExtractFloatsFromString(wdStringView sText, wdUInt32 uiNumFloats, float* out_pFloats,
    const char** out_pLastParsePosition = nullptr); // [tested]

  /// \brief Converts a hex character ('0', '1', ... '9', 'A'/'a', ... 'F'/'f') to the corresponding int value 0 - 15.
  ///
  /// \note Returns -1 for invalid HEX characters.
  WD_FOUNDATION_DLL wdInt8 HexCharacterToIntValue(wdUInt32 uiCharacter); // [tested]

  /// \brief Same as ConvertHexStringToUInt() with uiMaxHexCharacters set to 8.
  WD_FOUNDATION_DLL wdResult ConvertHexStringToUInt32(wdStringView sHex, wdUInt32& out_uiResult); // [tested]

  /// \brief Same as ConvertHexStringToUInt() with uiMaxHexCharacters set to 16.
  WD_FOUNDATION_DLL wdResult ConvertHexStringToUInt64(wdStringView sHex, wdUInt64& out_uiResult); // [tested]

  /// \brief Converts a hex string (i.e. 0xAABBCCDD) into its uint64 value.
  ///
  /// "0x" at the beginning is ignored.
  /// Empty strings are interpreted as 'valid', representing the value 0 (returns WD_SUCCESS).
  /// If the wdStringView is shorter than uiMaxHexCharacters, this is interpreted as a valid HEX value with a smaller value.
  /// If the string is longer than uiMaxHexCharacters (after the '0x'), the additional characters are not parsed, at all.
  /// If the first uiMaxHexCharacters (after the '0x') contain any non-HEX characters, parsing is interrupted and WD_FAILURE is returned.
  WD_FOUNDATION_DLL wdResult ConvertHexStringToUInt(wdStringView sHex, wdUInt64& out_uiResult, wdUInt32 uiMaxHexCharacters, wdUInt32* pTotalCharactersParsed); // [tested]
  ...
  WD_FOUNDATION_DLL void ConvertHexToBinary(wdStringView sText, wdUInt8* pBinary, wdUInt32 uiBinaryBuffer); // [tested]
  ...
  inline void ConvertBinaryToHex(const void* pBinaryData, wdUInt32 uiBytes, APPEND_CONTAINER_LAMBDA append); // [tested]

  /// \brief Converts a string that was written with wdConversionUtils::ToString(wdUuid) back to an wdUuid object.
  WD_FOUNDATION_DLL wdUuid ConvertStringToUuid(wdStringView sText); // [tested]
  ...
  WD_FOUNDATION_DLL bool IsStringUuid(wdStringView sText); // [tested]

  /// \brief Converts a bool to a string
  WD_ALWAYS_INLINE const wdStringBuilder& ToString(bool value, wdStringBuilder& out_sResult) // [tested]
  {
  ...
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdInt8 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 8bit unsigned integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdUInt8 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 16bit signed integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdInt16 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 16bit unsigned integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdUInt16 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 32bit signed integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdInt32 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 32bit unsigned integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdUInt32 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 64bit signed integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdInt64 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a 64bit unsigned integer to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(wdUInt64 value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a float to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(float value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a double to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(double value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a color to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdColor& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a color to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdColorGammaUB& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec2 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec2& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec3 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec3& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec4 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec4& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec2I32 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec2I32& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec3I32 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec3I32& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a vec4I32 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdVec4I32& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a quat to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdQuat& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a mat3 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdMat3& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a mat4 to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdMat4& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a transform to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdTransform& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts a Uuid to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdUuid& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts an angle to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdAngle& value, wdStringBuilder& out_sResult); // [tested]

  /// \brief Converts an angle to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdTime& value, wdStringBuilder& out_sResult);

  /// \brief Converts a wdStringView to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdStringView& value, wdStringBuilder& out_sResult);

  /// \brief Converts a wdVariantArray to a string
  WD_FOUNDATION_DLL const wdStringBuilder& ToString(const wdDynamicArray<wdVariant, wdDefaultAllocatorWrapper>& value, wdStringBuilder& out_sResult);

  /// \brief Fallback ToString implementation for all types that don't have one
  template <typename T>
  WD_ALWAYS_INLINE const wdStringBuilder& ToString(const T& value, wdStringBuilder& out_sResult)
  {
    out_sResult = "N/A";
    return out_sResult;
  }
  ...
  WD_FOUNDATION_DLL wdColor GetColorByName(wdStringView sText, bool* out_pValidColorName = nullptr); // [tested]

  /// \brief The inverse of GetColorByName
  WD_FOUNDATION_DLL wdString GetColorName(const wdColor& col); // [tested]
};                                                             // namespace wdConversionUtils

template <typename APPEND_CONTAINER_LAMBDA>
inline void wdConversionUtils::ConvertBinaryToHex(const void* pBinaryData, wdUInt32 uiBytes, APPEND_CONTAINER_LAMBDA append) // [tested]
{
  char tmp[4];

  wdUInt8* pBytes = (wdUInt8*)pBinaryData;
