...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand('type synthetic add -x "^nsDynamicArray<" --python-class nsEngine.nsDynamicArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^nsHybridArray<" --python-class nsEngine.nsHybridArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^nsHybridString<" --python-class nsEngine.nsHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add nsStringBuilder --python-class nsEngine.nsHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add nsStringView --python-class nsEngine.nsStringViewSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^nsEnum<" --python-class nsEngine.nsEnumSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^nsArrayPtr<" --python-class nsEngine.nsArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add nsByteArrayPtr --python-class nsEngine.nsArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add nsConstByteArrayPtr --python-class nsEngine.nsArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^nsMap<" --python-class nsEngine.nsMapSynthProvider')

    debugger.HandleCommand('type summary add -x "^nsHybridString<" --python-function nsEngine.nsHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add nsStringBuilder --python-function nsEngine.nsHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add nsStringView --python-function nsEngine.nsStringView_SummaryProvider')
    debugger.HandleCommand('type summary add -x "^nsEnum<" --python-function nsEngine.nsEnum_SummaryProvider')
...
class nsDynamicArraySynthProvider:
...
class nsHybridArraySynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
def nsHybridString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "nsHybridString_SummaryProvider"
...
class nsStringViewSynthProvider:
    ...
    def update(self):
        logger = lldb.formatters.Logger.Logger()
        logger >> "nsStringViewSynthProvider::update"
        self.valid = False
        try:
...
def nsStringView_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "nsStringView_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return '"' + make_string(content) + '"'
    except Exception as inst:
        logger >> str(inst)
...
class nsEnumSynthProvider:
    ...
    def update(self):
        self.value = "?"
        logger = lldb.formatters.Logger.Logger()
        logger >> "nsEnumSynthProvider::Update"
        try:
