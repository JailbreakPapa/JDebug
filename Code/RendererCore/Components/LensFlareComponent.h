...
class NS_RENDERERCORE_DLL nsLensFlareRenderData : public nsRenderData
{
  NS_ADD_DYNAMIC_REFLECTION(nsLensFlareRenderData, nsRenderData);

public:
  void FillBatchIdAndSortingKey();

  nsTexture2DResourceHandle m_hTexture;
  nsFloat16Vec4 m_Color;
...
NS_DECLARE_REFLECTABLE_TYPE(NS_RENDERERCORE_DLL, nsLensFlareElement);

using nsLensFlareComponentManager = nsComponentManager<class nsLensFlareComponent, nsBlockStorageType::Compact>;
...
class NS_RENDERERCORE_DLL nsLensFlareComponent : public nsRenderComponent
{
  NS_DECLARE_COMPONENT_TYPE(nsLensFlareComponent, nsRenderComponent, nsLensFlareComponentManager);
...
public:
  ...
  nsSmallArray<nsLensFlareElement, 1> m_Elements; // [ property ]
...
private:
  ...
  nsComponentHandle m_hLightComponent;
};
