...
nsReflectionProbeMapping::nsReflectionProbeMapping(nsUInt32 uiAtlasSize)
  : m_uiAtlasSize(uiAtlasSize)
{
  ...
  NS_ASSERT_DEV(m_hReflectionSpecularTexture.IsInvalidated(), "World data already created.");
...
nsReflectionProbeMapping::~nsReflectionProbeMapping()
{
  NS_ASSERT_DEV(!m_hReflectionSpecularTexture.IsInvalidated(), "World data not created.");
  nsGALDevice::GetDefaultDevice()->DestroyTexture(m_hReflectionSpecularTexture);
  m_hReflectionSpecularTexture.Invalidate();
}

void nsReflectionProbeMapping::AddProbe(nsReflectionProbeId probe, nsBitflags<nsProbeFlags> flags)
{
  ...
  NS_ASSERT_DEBUG(probeData.m_Flags == 0, "");
  probeData.m_id = probe;
  probeData.m_Flags.SetValue(flags.GetValue());
  probeData.m_Flags.Add(nsProbeMappingFlags::Dirty);
  if (probeData.m_Flags.IsSet(nsProbeMappingFlags::SkyLight))
  {
    m_SkyLight = probe;
    MapProbe(probe, 0);
  }
...
void nsReflectionProbeMapping::UpdateProbe(nsReflectionProbeId probe, nsBitflags<nsProbeFlags> flags)
{
  ProbeDataInternal& probeData = m_RegisteredProbes[probe.m_InstanceIndex];
  if (!probeData.m_Flags.IsSet(nsProbeMappingFlags::SkyLight) && probeData.m_Flags.IsSet(nsProbeMappingFlags::Dynamic) != flags.IsSet(nsProbeFlags::Dynamic))
  {
    UnmapProbe(probe);
  }
  nsBitflags<nsProbeMappingFlags> preserveFlags = probeData.m_Flags & nsProbeMappingFlags::Usable;
  probeData.m_Flags.SetValue(flags.GetValue());
  probeData.m_Flags.Add(preserveFlags | nsProbeMappingFlags::Dirty);
}

void nsReflectionProbeMapping::ProbeUpdateFinished(nsReflectionProbeId probe)
{
  ProbeDataInternal& probeData0 = m_RegisteredProbes[probe.m_InstanceIndex];
  if (m_SkyLight == probe && probeData0.m_Flags.IsSet(nsProbeMappingFlags::Dirty))
  {
  ...
  probeData0.m_Flags.Add(nsProbeMappingFlags::Usable);
  probeData0.m_Flags.Remove(nsProbeMappingFlags::Dirty);
}

void nsReflectionProbeMapping::RemoveProbe(nsReflectionProbeId probe)
{
  if (m_SkyLight == probe)
  {
...
nsInt32 nsReflectionProbeMapping::GetReflectionIndex(nsReflectionProbeId probe, bool bForExtraction) const
{
  const ProbeDataInternal& probeData = m_RegisteredProbes[probe.m_InstanceIndex];
  if (bForExtraction && !probeData.m_Flags.IsSet(nsProbeMappingFlags::Usable))
  {
    return -1;
  }
...
void nsReflectionProbeMapping::PreExtraction()
{
  // Reset priorities
  for (ProbeDataInternal& probeData : m_RegisteredProbes)
  {
  ...
  if (!m_SkyLight.IsInvalidated())
  {
    ...
    probeData.m_fPriority = nsMath::MaxValue<float>();
  }
...
void nsReflectionProbeMapping::AddWeight(nsReflectionProbeId probe, float fPriority)
{
  ProbeDataInternal& probeData = m_RegisteredProbes[probe.m_InstanceIndex];
  probeData.m_fPriority = nsMath::Max(probeData.m_fPriority, fPriority);
}

void nsReflectionProbeMapping::PostExtraction()
{
  {
    // Sort all active non-skylight probes so we can find the best candidates to evict from the atlas.
    for (nsUInt32 i = 1; i < s_uiNumReflectionProbeCubeMaps; i++)
    {
...

  {
    ...
    const nsUInt32 uiMaxCount = nsMath::Min(m_uiAtlasSize, m_SortedProbes.GetCount());
    for (nsUInt32 i = 0; i < uiMaxCount; i++)
    {
...

  {
    ...
    const nsUInt32 uiMaxCount = m_AddProbes.GetCount();
    for (nsUInt32 i = 0; i < uiMaxCount; i++)
    {
  ...
  {
    const nsUInt32 uiMaxCount = nsMath::Min(m_AddProbes.GetCount(), m_UnusedProbeSlots.GetCount());
    for (nsUInt32 i = 0; i < uiMaxCount; i++)
    {
      nsInt32 iReflectionIndex = m_UnusedProbeSlots[i];
      const SortedProbes probe = m_AddProbes[i];
  ...
  // Enqueue dynamic probe updates
  {
    ...
    if (!m_SkyLight.IsInvalidated())
    {
      m_ActiveProbes.PushBack({m_SkyLight, nsMath::MaxValue<float>()});
    }
    const nsUInt32 uiMaxCount = m_ActiveProbes.GetCount();
    for (nsUInt32 i = 0; i < uiMaxCount; i++)
    {
      const SortedProbes probe = m_ActiveProbes[i];
      const ProbeDataInternal& probeData = m_RegisteredProbes[probe.m_uiIndex.m_InstanceIndex];
      if (probeData.m_Flags.IsSet(nsProbeMappingFlags::Dynamic))
      {
        nsReflectionProbeMappingEvent e = {probeData.m_id, nsReflectionProbeMappingEvent::Type::ProbeUpdateRequested};
      ...
      else
      {
        ...
        if (probeData.m_Flags.IsSet(nsProbeMappingFlags::Dirty))
        {
          nsReflectionProbeMappingEvent e = {probeData.m_id, nsReflectionProbeMappingEvent::Type::ProbeUpdateRequested};
          m_Events.Broadcast(e);
        }
...
void nsReflectionProbeMapping::MapProbe(nsReflectionProbeId id, nsInt32 iReflectionIndex)
{
  ...
  nsReflectionProbeMappingEvent e = {id, nsReflectionProbeMappingEvent::Type::ProbeMapped};
  m_Events.Broadcast(e);
}

void nsReflectionProbeMapping::UnmapProbe(nsReflectionProbeId id)
{
  ProbeDataInternal& probeData = m_RegisteredProbes[id.m_InstanceIndex];
  if (probeData.m_uiReflectionIndex != -1)
  {
    ...
    nsReflectionProbeMappingEvent e = {id, nsReflectionProbeMappingEvent::Type::ProbeUnmapped};
    m_Events.Broadcast(e);
  }
}
