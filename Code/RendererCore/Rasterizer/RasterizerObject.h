#pragma once

#include <Foundation/Math/Mat4.h>
#include <Foundation/Strings/String.h>
#include <Foundation/Types/RefCounted.h>
#include <Foundation/Types/SharedPtr.h>
#include <RendererCore/Rasterizer/Thirdparty/Occluder.h>
#include <RendererCore/RendererCoreDLL.h>

class nsGeometry;

class NS_RENDERERCORE_DLL nsRasterizerObject : public nsRefCounted
{
  NS_DISALLOW_COPY_AND_ASSIGN(nsRasterizerObject);

public:
  nsRasterizerObject();
  ~nsRasterizerObject();

  /// \brief If an object with the given name has been created before, it is returned, otherwise nullptr is returned.
  ///
  /// Use this to quickly query for an existing object. Call CreateMesh() in case the object doesn't exist yet.
  static nsSharedPtr<const nsRasterizerObject> GetObject(nsStringView sUniqueName);

  /// \brief Creates a box object with the specified dimensions. If such a box was created before, the same pointer is returned.
  static nsSharedPtr<const nsRasterizerObject> CreateBox(const nsVec3& vFullExtents);

  /// \brief Creates an object with the given geometry. If an object with the same name was created before, that pointer is returned instead.
  ///
  /// It is assumed that the same name will only be used for identical geometry.
  static nsSharedPtr<const nsRasterizerObject> CreateMesh(nsStringView sUniqueName, const nsGeometry& geometry);

private:
  void CreateMesh(const nsGeometry& geometry);

  friend class nsRasterizerView;
  Occluder m_Occluder;

  static nsMutex s_Mutex;
  static nsMap<nsString, nsSharedPtr<nsRasterizerObject>> s_Objects;
};
