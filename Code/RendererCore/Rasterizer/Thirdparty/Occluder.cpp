...
Occluder::~Occluder()
{
  NS_DELETE_RAW_BUFFER(nsFoundation::GetAlignedAllocator(), m_vertexData);
...
void Occluder::bake(const __m128* vertices, nsUInt32 numVertices, __m128 refMin, __m128 refMax)
{
  assert(numVertices % 16 == 0);

  // Simple k-means clustering by normal direction to improve backface culling efficiency
  nsHybridArray<__m128, 32, nsAlignedAllocatorWrapper> quadNormals;
  for (nsUInt32 i = 0; i < numVertices; i += 4)
  {
  ...
  nsHybridArray<__m128, 32, nsAlignedAllocatorWrapper> centroids;
  nsHybridArray<uint32_t, 32> centroidAssignment;
  ...
  nsHybridArray<__m128, 64, nsAlignedAllocatorWrapper> orderedVertices;
  for (uint32_t k = 0; k < centroids.GetCount(); ++k)
  {
    for (nsUInt32 j = 0; j < numVertices / 4; ++j)
    {
  ...
  occluder->m_vertexData = NS_NEW_RAW_BUFFER(nsFoundation::GetAlignedAllocator(), __m256i, orderedVertices.GetCount() * 4);

  for (nsUInt32 i = 0; i < orderedVertices.GetCount(); i += 32)
  {
    __m128i v[8];
  ...
  for (nsUInt32 i = 0; i < orderedVertices.GetCount(); ++i)
  {
