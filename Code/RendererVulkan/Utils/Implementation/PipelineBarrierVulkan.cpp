...
namespace
{
  vk::ImageSubresourceRange CreateSubRange(const vk::ImageSubresourceRange& fullRange, nsUInt32 uiLayer, nsUInt32 uiMipLevel)
  {
...
void nsPipelineBarrierVulkan::SetCommandBuffer(vk::CommandBuffer* pCommandBuffer)
{
  m_pCommandBuffer = pCommandBuffer;
}

void nsPipelineBarrierVulkan::Flush()
{
  if (m_srcStageMask || m_dstStageMask)
  {
    ...
    nsLog::Warning("Flush: [m{}] {}|{} -> {}|{}", bHasMemoryBarrier, vk::to_string(m_srcStageMask).c_str(), vk::to_string(m_srcAccess).c_str(), vk::to_string(m_dstStageMask).c_str(), vk::to_string(m_dstAccess).c_str());

    for (vk::ImageMemoryBarrier& img : m_imageBarriers)
    {
      nsLog::Warning("Layout Changed: {} [{},{}]: {} [{}] -> {} [{}]", nsArgP(img.image), img.subresourceRange.baseMipLevel, img.subresourceRange.levelCount, vk::to_string(img.oldLayout).c_str(), vk::to_string(img.srcAccessMask).c_str(), vk::to_string(img.newLayout).c_str(), vk::to_string(img.dstAccessMask).c_str());
    }
#endif
...
void nsPipelineBarrierVulkan::Submit()
{
  Flush();
  ...
  for (auto it = m_imageState.GetIterator(); it.IsValid();)
  {
    ...
    const nsUInt32 uiCount = it.Value().m_subElementLayout.GetCount();
...
bool nsPipelineBarrierVulkan::IsDirty(vk::Image image, const vk::ImageSubresourceRange& subResources) const
{
...
bool nsPipelineBarrierVulkan::IsDirty() const
{
  return m_srcStageMask || m_dstStageMask;
}

bool nsPipelineBarrierVulkan::IsDirty(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::AccessFlags dstAccess)
{
...
void nsPipelineBarrierVulkan::AccessBuffer(const nsGALBufferVulkan* pBuffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  SubBufferState subState;
  subState.m_offset = offset;
  subState.m_length = length;
...
void nsPipelineBarrierVulkan::EnsureImageLayout(const nsGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  EnsureImageLayout(pTexture, pTexture->GetFullRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void nsPipelineBarrierVulkan::EnsureImageLayout(const nsGALRenderTargetViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const nsGALTextureVulkan*>(pTextureView->GetTexture()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void nsPipelineBarrierVulkan::EnsureImageLayout(const nsGALTextureResourceViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const nsGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void nsPipelineBarrierVulkan::EnsureImageLayout(const nsGALTextureUnorderedAccessViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const nsGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void nsPipelineBarrierVulkan::EnsureImageLayout(const nsGALTextureVulkan* pTexture, vk::ImageSubresourceRange subResources, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  const vk::ImageSubresourceRange fullRange = pTexture->GetFullRange();
  if (nsGALResourceFormat::IsStencilFormat(pTexture->GetDescription().m_Format))
  {
  ...
  if (pState)
  {
    ...
    if (bIsFullRange)
    {
      ...
      else
      {
        ...
        const nsUInt32 uiLastLayer = subResources.baseArrayLayer + subResources.layerCount;
        const nsUInt32 uiLastLevel = subResources.baseMipLevel + subResources.levelCount;
        for (nsUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
        {
          for (nsUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
          {
            const nsUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * fullRange.levelCount;
...
void nsPipelineBarrierVulkan::SetInitialImageState(const nsGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  ...
  NS_ASSERT_DEBUG(!it.IsValid() || it.Value().m_pTexture->GetDescription().m_pExisitingNativeObject != nullptr, "Can't set initial state, texture is already tracked.");
...
void nsPipelineBarrierVulkan::TextureDestroyed(const nsGALTextureVulkan* pTexture)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  if (it.IsValid())
  {
    ...
    if (it.Value().m_dirty.IsAnyBitSet())
    {
      ...
      for (nsInt32 i = ((nsInt32)m_imageBarriers.GetCount()) - 1; i >= 0; --i)
      {
...
void nsPipelineBarrierVulkan::BufferDestroyed(const nsGALBufferVulkan* pBuffer)
{
...
void nsPipelineBarrierVulkan::FullBarrier()
{
  vk::MemoryBarrier memoryBarrier;
...
bool nsPipelineBarrierVulkan::AddBufferBarrierInternal(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
...
bool nsPipelineBarrierVulkan::IsDirtyInternal(const BufferState& state, const SubBufferState& subState) const
{
  const nsUInt32 uiCount = state.m_subBufferState.GetCount();
  for (nsUInt32 i = 0; i < uiCount; i++)
  {
...
bool nsPipelineBarrierVulkan::AddImageBarrierInternal(vk::Image image, const vk::ImageSubresourceRange& subResources, vk::ImageLayout srcLayout, vk::AccessFlags srcAccess, vk::ImageLayout dstLayout, vk::AccessFlags dstAccess, bool bDiscardSource)
{
...
bool nsPipelineBarrierVulkan::IsDirtyInternal(const ImageState& state, const vk::ImageSubresourceRange& subResources) const
{
  ...
  {
    nsUInt32 uiLevelCount = state.m_pTexture->GetDescription().m_uiMipLevelCount;
    ...
    for (nsUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
    {
      for (nsUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
      {
        const nsUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * uiLevelCount;
