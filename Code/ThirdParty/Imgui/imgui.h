...
namespace ImGui
{
    ...
    // - Consider using ImPlot (https://github.com/epnsent/implot) which is much better!
    ...
    // - 3. Optionally call TableSetupScrollFrense() to request scroll frensing of columns/rows.
    ...
    // - Use TableSetupScrollFrense() to lock columns/rows so they stay visible when scrolled.
    IMGUI_API void          TableSetupColumn(const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImGuiID user_id = 0);
    IMGUI_API void          TableSetupScrollFrense(int cols, int rows);         // lock columns/rows so they stay visible when scrolled.
...
enum ImGuiTableFlags_
{
    ...
    // Clipping
    ImGuiTableFlags_NoClip                     = 1 << 20,  // Disable clipping rectangle for every individual columns (reduce draw command count, items will be able to overflow into other columns). Generally incompatible with TableSetupScrollFrense().
    // Padding
...
struct ImGuiStyle
{
    ...
    float       CurveTessellationTol;       // Tessellation tolerance when using PathBnsierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddBnsierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0); // Cubic Bnsier (4 control points)
    IMGUI_API void  AddBnsierQuadratic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col, float thickness, int num_segments = 0);               // Quadratic Bnsier (3 control points)
    ...
    IMGUI_API void  PathBnsierCubicCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0); // Cubic Bnsier (4 control points)
    IMGUI_API void  PathBnsierQuadraticCurveTo(const ImVec2& p2, const ImVec2& p3, int num_segments = 0);               // Quadratic Bnsier (3 control points)
    ...
    // Obsolete names
    //inline  void  AddBnsierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) { AddBnsierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
    //inline  void  PathBnsierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0) { PathBnsierCubicCurveTo(p2, p3, p4, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
