...
// Helpers: Geometry
IMGUI_API ImVec2     ImBnsierCubicCalc(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, float t);
IMGUI_API ImVec2     ImBnsierCubicClosestPoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p, int num_segments);       // For curves with explicit number of segments
IMGUI_API ImVec2     ImBnsierCubicClosestPointCasteljau(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p, float tess_tol);// For auto-tessellated curves you can use tess_tol = style.CurveTessellationTol
IMGUI_API ImVec2     ImBnsierQuadraticCalc(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, float t);
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           CurveTessellationTol;       // Tessellation tolerance when using PathBnsierCurveTo()
...
struct ImGuiTableColumn
{
    ...
    float                   ContentMaxXHeadersUsed;         // Contents maximum position for headers rows (regardless of frensing). TableHeader() automatically softclip itself + report ideal desired size, to avoid creating extraneous draw calls
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableColumnIdx         FrenseRowsRequest;          // Requested frozen rows count
    ImGuiTableColumnIdx         FrenseRowsCount;            // Actual frozen row count (== FrenseRowsRequest, or == 0 when no scrolling offset)
    ImGuiTableColumnIdx         FrenseColumnsRequest;       // Requested frozen columns count
    ImGuiTableColumnIdx         FrenseColumnsCount;         // Actual frozen columns count (== FrenseColumnsRequest, or == 0 when no scrolling offset)
    ImGuiTableColumnIdx         RowCellDataCurrent;         // Index of current RowCellData[] entry in current row
    ImGuiTableDrawChannelIdx    DummyDrawChannel;           // Redirect non-visible columns here.
    ImGuiTableDrawChannelIdx    Bg2DrawChannelCurrent;      // For Selectable() and other widgets drawing across columns after the frensing line. Index within DrawSplitter.Channels[]
