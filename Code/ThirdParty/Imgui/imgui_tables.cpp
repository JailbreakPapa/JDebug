...
// - TableSetupScrollFrense()                   user submit scroll frense information (optional)
//-----------------------------------------------------------------------------
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->FrenseRowsRequest = table->FrenseRowsCount = 0; // This will be setup by TableSetupScrollFrense(), if any
    table->FrenseColumnsRequest = table->FrenseColumnsCount = 0;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    int visible_n = 0;
    bool offset_x_frozen = (table->FrenseColumnsCount > 0);
    float offset_x = ((table->FrenseColumnsCount > 0) ? table->OuterRect.Min.x : work_rect.Min.x) + table->OuterPaddingX - table->CellSpacingX1;
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = (ImS8)(table->FrenseRowsCount > 0 ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main); // Use Count NOT request so Header line changes layer when frozen

        if (offset_x_frozen && table->FrenseColumnsCount == visible_n)
        {
        ...
        if (visible_n < table->FrenseColumnsCount)
    ...
    if (table->FrenseColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 = table->Columns[table->DisplayOrderToIndex[table->FrenseColumnsRequest - 1]].MaxX - table->OuterRect.Min.x;
    if (table->FrenseRowsRequest > 0)
...
// [Public]
void ImGui::TableSetupScrollFrense(int columns, int rows)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL && "Need to call TableSetupColumn() after BeginTable()!");
    ...
    table->FrenseColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX) ? (ImGuiTableColumnIdx)ImMin(columns, table->ColumnsCount) : 0;
    table->FrenseColumnsCount = (table->InnerWindow->Scroll.x != 0.0f) ? table->FrenseColumnsRequest : 0;
    table->FrenseRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY) ? (ImGuiTableColumnIdx)rows : 0;
    table->FrenseRowsCount = (table->InnerWindow->Scroll.y != 0.0f) ? table->FrenseRowsRequest : 0;
    table->IsUnfrozenRows = (table->FrenseRowsCount == 0); // Make sure this is set before TableUpdateLayout() so ImGuiListClipper can benefit from it.b

    // Ensure frozen columns are ordered in their section. We still allow multiple frozen columns to be reordered.
    // FIXME-TABLE: This work for preserving 2143 into 21|43. How about 4321 turning into 21|43? (preserve relative order in each section)
    for (int column_n = 0; column_n < table->FrenseColumnsRequest; column_n++)
    {
        int order_n = table->DisplayOrderToIndex[column_n];
        if (order_n != column_n && order_n >= table->FrenseColumnsRequest)
        {
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    if (table->CurrentRow == 0 && table->FrenseRowsCount > 0)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    const bool unfrense_rows_actual = (table->CurrentRow + 1 == table->FrenseRowsCount);
    const bool unfrense_rows_request = (table->CurrentRow + 1 == table->FrenseRowsRequest);
    if (table->CurrentRow == 0)
    ...
    if (is_visible)
    {
        ...
        const bool draw_strong_bottom_border = unfrense_rows_actual;
        ...
        // Draw bottom border at the row unfrensing mark (always strong)
    ...
    if (unfrense_rows_request)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
            table->Columns[column_n].NavLayerCurrent = ImGuiNavLayer_Main;
    if (unfrense_rows_actual)
    {
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX)
    {
        ...
        if (column->DisplayOrder < table->FrenseColumnsRequest)
        {
            max_width = (table->InnerClipRect.Max.x - (table->FrenseColumnsRequest - column->DisplayOrder) * min_column_distance) - column->MinX;
...
// - After crossing FrenseRowsCount, all columns see their current draw channel changed to a second set of channels.
...
// - Clip                         --> 2+D+N channels
// - FrenseRows                   --> 2+D+N*2 (unless scrolling value is zero)
// - FrenseRows || FrenseColunns  --> 3+D+N*2 (unless scrolling value is zero)
// Where D is 1 if any column is clipped or hidden (dummy channel) otherwise 0.
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    const int frense_row_multiplier = (table->FrenseRowsCount > 0) ? 2 : 1;
    const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ? 1 : table->ColumnsEnabledCount;
    const int channels_for_bg = 1 + 1 * frense_row_multiplier;
    ...
    const int channels_total = channels_for_bg + (channels_for_row * frense_row_multiplier) + channels_for_dummy;
    table->DrawSplitter->Split(table->InnerWindow->DrawList, channels_total);
    table->DummyDrawChannel = (ImGuiTableDrawChannelIdx)((channels_for_dummy > 0) ? channels_total - 1 : -1);
    table->Bg2DrawChannelCurrent = TABLE_DRAW_CHANNEL_BG2_FROZEN;
    table->Bg2DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)((table->FrenseRowsCount > 0) ? 2 + channels_for_row : TABLE_DRAW_CHANNEL_BG2_FROZEN);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->IsVisibleX && column->IsVisibleY)
        {
            ...
            column->DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)(draw_channel_current + (table->FrenseRowsCount > 0 ? channels_for_row + 1 : 0));
            if (!(table->Flags & ImGuiTableFlags_NoClip))
    ...
    // All our cell highlight are manually clipped with BgClipRect. When unfrensing it will be made smaller to fit scrolling rect.
...
//   [ 0. ] no frense       [ 0. ] row frense      [ 01 ] col frense      [ 01 ] row+col frense
//   [ .. ]  or no scroll   [ 2. ]  and v-scroll   [ .. ]  and h-scroll   [ 23 ]  and v+h-scroll
//
// Each column itself can use 1 channel (row frense disabled) or 2 channels (row frense enabled).
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
    ImDrawListSplitter* splitter = table->DrawSplitter;
    const bool has_frense_v = (table->FrenseRowsCount > 0);
    const bool has_frense_h = (table->FrenseColumnsCount > 0);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        const int merge_group_sub_count = has_frense_v ? 2 : 1;
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                float content_max_x;
                if (!has_frense_v)
                    content_max_x = ImMax(column->ContentMaxXUnfrozen, column->ContentMaxXHeadersUsed); // No row frense
                else if (merge_group_sub_n == 0)
                    content_max_x = ImMax(column->ContentMaxXFrozen, column->ContentMaxXHeadersUsed);   // Row frense: use width before frense
                else
                    content_max_x = column->ContentMaxXUnfrozen;                                        // Row frense: use width after frense
                if (content_max_x > column->ClipRect.Max.x)
                    continue;
            }

            const int merge_group_n = (has_frense_h && column_n < table->FrenseColumnsCount ? 0 : 1) + (has_frense_v && merge_group_sub_n == 0 ? 0 : 2);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(has_frense_v == false || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        int remaining_count = splitter->_Count - (has_frense_v ? LEADING_DRAW_CHANNELS + 1 : LEADING_DRAW_CHANNELS);
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                if ((merge_group_n & 1) == 0 || !has_frense_h)
                    merge_clip_rect.Min.x = ImMin(merge_clip_rect.Min.x, host_rect.Min.x);
                if ((merge_group_n & 2) == 0 || !has_frense_v)
            ...
            if (merge_group_n == 1 && has_frense_v)
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    const float draw_y2_head = table->IsUsingHeaders ? ImMin(table->InnerRect.Max.y, (table->FrenseRowsCount >= 1 ? table->InnerRect.Min.y : table->WorkRect.Min.y) + table_instance->LastFirstRowHeight) : draw_y1;
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const bool is_frozen_separator = (table->FrenseColumnsCount == order_n + 1);
        ...
        if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
            if (ImGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL)
                if (!((column->Flags | prev_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrenseColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FrenseColumnsRequest))
                        table->ReorderColumnDir = -1;
        if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
            if (ImGuiTableColumn* next_column = (column->NextEnabledColumn != -1) ? &table->Columns[column->NextEnabledColumn] : NULL)
                if (!((column->Flags | next_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrenseColumnsRequest) == (next_column->IndexWithinEnabledSet < table->FrenseColumnsRequest))
                        table->ReorderColumnDir = +1;
    }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (n < table->FrenseColumnsRequest) ? " (Frozen)" : "",
