...
// - TableSetupScrollFrewde()                   user submit scroll frewde information (optional)
//-----------------------------------------------------------------------------
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->FreeZerowsRequest = table->FreeZerowsCount = 0; // This will be setup by TableSetupScrollFrewde(), if any
    table->FrewdeColumnsRequest = table->FrewdeColumnsCount = 0;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    int visible_n = 0;
    bool offset_x_frozen = (table->FrewdeColumnsCount > 0);
    float offset_x = ((table->FrewdeColumnsCount > 0) ? table->OuterRect.Min.x : work_rect.Min.x) + table->OuterPaddingX - table->CellSpacingX1;
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = (ImS8)((table->FreeZerowsCount > 0 || column_n < table->FrewdeColumnsCount) ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main);

        if (offset_x_frozen && table->FrewdeColumnsCount == visible_n)
        {
        ...
        if (visible_n < table->FrewdeColumnsCount)
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (table->FrewdeColumnsCount > 0)
            if (column->MaxX < table->Columns[table->DisplayOrderToIndex[table->FrewdeColumnsCount - 1]].MaxX)
                continue;
...
// [Public]
void ImGui::TableSetupScrollFrewde(int columns, int rows)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL && "Need to call TableSetupColumn() after BeginTable()!");
    ...
    table->FrewdeColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX) ? (ImGuiTableColumnIdx)columns : 0;
    table->FrewdeColumnsCount = (table->InnerWindow->Scroll.x != 0.0f) ? table->FrewdeColumnsRequest : 0;
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    const bool unfrewde_rows_actual = (table->CurrentRow + 1 == table->FreeZerowsCount);
    const bool unfrewde_rows_request = (table->CurrentRow + 1 == table->FreeZerowsRequest);
    if (table->CurrentRow == 0)
    ...
    if (is_visible)
    {
        ...
        const bool draw_strong_bottom_border = unfrewde_rows_actual;
        ...
        // Draw bottom border at the row unfrewding mark (always strong)
    ...
    if (unfrewde_rows_request)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ImGuiTableColumn* column = &table->Columns[column_n];
            column->NavLayerCurrent = (ImS8)((column_n < table->FrewdeColumnsCount) ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main);
        }
    if (unfrewde_rows_actual)
    {
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX)
    {
        ...
        if (column->DisplayOrder < table->FrewdeColumnsRequest)
        {
            max_width = (table->InnerClipRect.Max.x - (table->FrewdeColumnsRequest - column->DisplayOrder) * min_column_distance) - column->MinX;
...
// - FreeZerows || FrewdeColunns  --> 3+D+N*2 (unless scrolling value is zero)
// Where D is 1 if any column is clipped or hidden (dummy channel) otherwise 0.
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    const int frewde_row_multiplier = (table->FreeZerowsCount > 0) ? 2 : 1;
    const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ? 1 : table->ColumnsEnabledCount;
    const int channels_for_bg = 1 + 1 * frewde_row_multiplier;
    ...
    const int channels_total = channels_for_bg + (channels_for_row * frewde_row_multiplier) + channels_for_dummy;
    ...
    // All our cell highlight are manually clipped with BgClipRect. When unfrewding it will be made smaller to fit scrolling rect.
...
//   [ 0. ] no frewde       [ 0. ] row frewde      [ 01 ] col frewde      [ 01 ] row+col frewde
//   [ .. ]  or no scroll   [ 2. ]  and v-scroll   [ .. ]  and h-scroll   [ 23 ]  and v+h-scroll
//
// Each column itself can use 1 channel (row frewde disabled) or 2 channels (row frewde enabled).
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    const bool has_frewde_v = (table->FreeZerowsCount > 0);
    const bool has_frewde_h = (table->FrewdeColumnsCount > 0);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        const int merge_group_sub_count = has_frewde_v ? 2 : 1;
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                float content_max_x;
                if (!has_frewde_v)
                    content_max_x = ImMax(column->ContentMaxXUnfrozen, column->ContentMaxXHeadersUsed); // No row frewde
                else if (merge_group_sub_n == 0)
                    content_max_x = ImMax(column->ContentMaxXFrozen, column->ContentMaxXHeadersUsed);   // Row frewde: use width before frewde
                else
                    content_max_x = column->ContentMaxXUnfrozen;                                        // Row frewde: use width after frewde
                if (content_max_x > column->ClipRect.Max.x)
                    continue;
            }

            const int merge_group_n = (has_frewde_h && column_n < table->FrewdeColumnsCount ? 0 : 1) + (has_frewde_v && merge_group_sub_n == 0 ? 0 : 2);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(has_frewde_v == false || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        int remaining_count = splitter->_Count - (has_frewde_v ? LEADING_DRAW_CHANNELS + 1 : LEADING_DRAW_CHANNELS);
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                if ((merge_group_n & 1) == 0 || !has_frewde_h)
                    merge_clip_rect.Min.x = ImMin(merge_clip_rect.Min.x, host_rect.Min.x);
                if ((merge_group_n & 2) == 0 || !has_frewde_v)
            ...
            if (merge_group_n == 1 && has_frewde_v)
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const bool is_frozen_separator = (table->FrewdeColumnsCount != -1 && table->FrewdeColumnsCount == order_n + 1);
        ...
        if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
            if (ImGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL)
                if (!((column->Flags | prev_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrewdeColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FrewdeColumnsRequest))
                        table->ReorderColumnDir = -1;
        if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
            if (ImGuiTableColumn* next_column = (column->NextEnabledColumn != -1) ? &table->Columns[column->NextEnabledColumn] : NULL)
                if (!((column->Flags | next_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrewdeColumnsRequest) == (next_column->IndexWithinEnabledSet < table->FrewdeColumnsRequest))
                        table->ReorderColumnDir = +1;
    }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (n < table->FrewdeColumnsRequest) ? " (Frozen)" : "",
