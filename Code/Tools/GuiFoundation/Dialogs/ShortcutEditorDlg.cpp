...
wdQtShortcutEditorDlg::wdQtShortcutEditorDlg(QWidget* pParent)
  : QDialog(pParent)
{
  setupUi(this);

  WD_VERIFY(connect(Shortcuts, SIGNAL(itemSelectionChanged()), this, SLOT(SlotSelectionChanged())) != nullptr, "signal/slot connection failed");

  m_iSelectedAction = -1;
  KeyEditor->setEnabled(false);

  wdMap<wdString, wdMap<wdString, wdInt32>> SortedItems;

  {
    auto itActions = wdActionManager::GetActionIterator();

    while (itActions.IsValid())
    {
      if (itActions.Value()->m_Type == wdActionType::Action)
      {
  ...
  {
    wdQtScopedBlockSignals bs(Shortcuts);
    wdQtScopedUpdatesDisabled ud(Shortcuts);
    ...
    for (auto it = SortedItems.GetIterator(); it.IsValid(); ++it)
    {
      ...
      for (auto it2 : it.Value())
      {
        ...
        sTemp = wdTranslate(item->m_sActionName);
        sTemp.ReplaceAll("&", "");
        ...
        pItem->setData(3, Qt::DisplayRole, wdTranslateTooltip(item->m_sActionName));
...
wdQtShortcutEditorDlg::~wdQtShortcutEditorDlg()
{
  wdActionManager::SaveShortcutAssignment();
}

void wdQtShortcutEditorDlg::UpdateTable()
{
  for (wdInt32 iTop = 0; iTop < Shortcuts->topLevelItemCount(); ++iTop)
  {
    auto pTopItem = Shortcuts->topLevelItem(iTop);

    for (wdInt32 iChild = 0; iChild < pTopItem->childCount(); ++iChild)
    {
      auto pChild = pTopItem->child(iChild);

      wdInt32 idx = pChild->data(0, Qt::UserRole).toInt();
...
void wdQtShortcutEditorDlg::SlotSelectionChanged()
{
...
void wdQtShortcutEditorDlg::on_KeyEditor_editingFinished()
{
  UpdateKeyEdit();
}

void wdQtShortcutEditorDlg::UpdateKeyEdit()
{
  if (m_iSelectedAction < 0)
    return;
...
void wdQtShortcutEditorDlg::on_KeyEditor_keySequenceChanged(const QKeySequence& keySequence)
{
  UpdateKeyEdit();
}

void wdQtShortcutEditorDlg::on_ButtonAssign_clicked()
{
  QString sText = KeyEditor->keySequence().toString(QKeySequence::SequenceFormat::NativeText);
  KeyEditor->clear();
...
void wdQtShortcutEditorDlg::on_ButtonRemove_clicked()
{
  KeyEditor->clear();
...
void wdQtShortcutEditorDlg::on_ButtonReset_clicked()
{
  KeyEditor->clear();
