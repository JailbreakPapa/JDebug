...
class WD_GUIFOUNDATION_DLL wdQtImageCache : public QObject
{
  Q_OBJECT

  WD_DECLARE_SINGLETON(wdQtImageCache);
...
public:
  ...
  const QPixmap* QueryPixmap(const char* szAbsolutePath, QModelIndex index = QModelIndex(), QVariant userData1 = QVariant(),
    QVariant userData2 = QVariant(), wdUInt32* out_pImageID = nullptr);
  ...
  void SetMemoryUsageThreshold(wdUInt64 uiMemoryThreshold) { m_iMemoryUsageThreshold = (wdInt64)uiMemoryThreshold; }
...
private:
  ...
  wdSet<Request> m_Requests;
  bool m_bCacheEnabled;
  bool m_bTaskRunning;
  wdTime m_LastCleanupTime;
  wdInt64 m_iMemoryUsageThreshold;
  wdInt64 m_iCurrentMemoryUsage;
  QPixmap* m_pImageLoading;
  QPixmap* m_pImageUnavailable;
  wdUInt32 m_uiCurImageID;

  struct CacheEntry
  {
    QPixmap m_Pixmap;
    wdTime m_LastAccess;
    wdUInt32 m_uiImageID;

    CacheEntry() { m_uiImageID = 0xFFFFFFFF; }
  };

  wdMap<QString, CacheEntry> m_ImageCache;
  wdMap<QString, QPixmap> m_TypeImages;
};

constexpr static wdUInt32 wdThumbnailSize = 256;

WD_ALWAYS_INLINE QPixmap wdSvgThumbnailToPixmap(const char* szFilePath)
{
  return QIcon(szFilePath).pixmap(QSize(wdThumbnailSize, wdThumbnailSize));
}
