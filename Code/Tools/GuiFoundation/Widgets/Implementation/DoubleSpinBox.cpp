...
wdQtDoubleSpinBox::wdQtDoubleSpinBox(QWidget* pParent, bool bIntMode)
  : QDoubleSpinBox(pParent)
{
  m_fDefaultValue = 0.0;
  m_fDisplayedValue = wdMath::NaN<float>();
  m_bInvalid = false;
  m_bModified = false;
  m_bIntMode = bIntMode;
  m_bDragging = false;
  ...
  connect(this, &QWidget::customContextMenuRequested, this, &wdQtDoubleSpinBox::onCustomContextMenuRequested);
}

void wdQtDoubleSpinBox::SetIntMode(bool bEnable)
{
  m_bIntMode = bEnable;
}

void wdQtDoubleSpinBox::setDisplaySuffix(const char* szSuffix)
{
  m_sSuffix = QString::fromUtf8(szSuffix);
}

void wdQtDoubleSpinBox::setDefaultValue(double value)
{
  m_fDefaultValue = value;
}


void wdQtDoubleSpinBox::setDefaultValue(const wdVariant& val)
{
  double fValue = 0;
  if (wdToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setDefaultValue(fValue);
}

void wdQtDoubleSpinBox::setMinimum(const wdVariant& val)
{
  double fValue = 0;
  if (wdToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMinimum(fValue);
...
void wdQtDoubleSpinBox::setMaximum(const wdVariant& val)
{
  double fValue = 0;
  if (wdToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMaximum(fValue);
...
QString wdQtDoubleSpinBox::textFromValue(double fVal) const
{
  if (m_bInvalid)
    return QString();

  if (hasFocus() && fVal == m_fDisplayedValue && !wdMath::IsNaN(m_fDisplayedValue))
  {
  ...
  if (m_bIntMode)
    fVal = wdMath::Round(QDoubleSpinBox::value());
...
double wdQtDoubleSpinBox::valueFromText(const QString& sText) const
{
  if (m_bInvalid)
  {
    m_bInvalid = false;
...
void wdQtDoubleSpinBox::setValueInvalid()
{
  m_bInvalid = true;
...
void wdQtDoubleSpinBox::setValue(double fVal)
{
  WD_ASSERT_DEBUG(wdMath::IsFinite(fVal), "Spin box value must be finite!");
  m_bInvalid = false;
  m_fDisplayedValue = wdMath::NaN<float>();
  QDoubleSpinBox::setValue(fVal);
}

void wdQtDoubleSpinBox::setValue(const wdVariant& val)
{
  double fValue = 0;
  if (wdToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setValue(fValue);
...
double wdQtDoubleSpinBox::value() const
{
  if (m_bInvalid)
    return 0.0;

  WD_ASSERT_DEBUG(!wdMath::IsNaN(QDoubleSpinBox::value()), "Spin box valid value should never be NaN!");
  return m_bIntMode ? wdMath::Round(QDoubleSpinBox::value()) : QDoubleSpinBox::value();
}

void wdQtDoubleSpinBox::focusInEvent(QFocusEvent* event)
{
...
void wdQtDoubleSpinBox::focusOutEvent(QFocusEvent* event)
{
  QDoubleSpinBox::focusOutEvent(event);
}

void wdQtDoubleSpinBox::mousePressEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void wdQtDoubleSpinBox::mouseReleaseEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void wdQtDoubleSpinBox::mouseMoveEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
    if (m_bDragging)
    {
      ...
      {
        m_LastDragPos = event->globalPos();
        const QRect dsize = wdWidgetUtils::GetClosestScreen(event->globalPos()).availableGeometry();
...
void wdQtDoubleSpinBox::keyPressEvent(QKeyEvent* event)
{
...
bool wdQtDoubleSpinBox::event(QEvent* event)
{
...
void wdQtDoubleSpinBox::onCustomContextMenuRequested()
{
  if (!isReadOnly())
  {
