...
wdQGridBarWidget::wdQGridBarWidget(QWidget* pParent)
  : QWidget(pParent)
{
...
void wdQGridBarWidget::SetConfig(
  const QRectF& viewportSceneRect, double fTextGridStops, double fFineGridStops, wdDelegate<QPointF(const QPointF&)> mapFromSceneFunc)
{
...
void wdQGridBarWidget::paintEvent(QPaintEvent* e)
{
  ...
  // render fine grid stop lines
  {
    ...
    wdWidgetUtils::ComputeGridExtentsX(m_ViewportSceneRect, m_fFineGridStops, fSceneMinX, fSceneMaxX);
    fSceneMinX = wdMath::Max(fSceneMinX, 0.0);
  ...
  {
    double fSceneMinX, fSceneMaxX;
    wdWidgetUtils::ComputeGridExtentsX(m_ViewportSceneRect, m_fTextGridStops, fSceneMinX, fSceneMaxX);
    fSceneMinX = wdMath::Max(fSceneMinX, 0.0);
