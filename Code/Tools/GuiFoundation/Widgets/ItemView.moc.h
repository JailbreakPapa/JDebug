...
/// \brief In combination with wdQtItemView this delegate allows for receiving the full range of mouse input.
class WD_GUIFOUNDATION_DLL wdQtItemDelegate : public QItemDelegate
{
  Q_OBJECT
public:
  explicit wdQtItemDelegate(QObject* pParent = nullptr)
...
/// \brief Template to be used with classes derived from QAbstractItemView. Allows the use of wdQtItemDelegate.
template <typename Base>
class wdQtItemView : public Base
{
public:
  wdQtItemView(QWidget* pParent)
    : Base(pParent)
...
private:
  bool ForwardEvent(QModelIndex index, QEvent* pEvent)
  {
    ...
    if (wdQtItemDelegate* pDelegate = qobject_cast<wdQtItemDelegate*>(this->itemDelegate(m_Hovered)))
    {
...
private:
  wdQtItemDelegate* m_pFocusedDelegate;
