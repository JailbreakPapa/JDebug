...
nsQtScopedUpdatesDisabled::nsQtScopedUpdatesDisabled(
  QWidget* pWidget1, QWidget* pWidget2, QWidget* pWidget3, QWidget* pWidget4, QWidget* pWidget5, QWidget* pWidget6)
{
  ...
  for (int i = 0; i < NS_ARRAY_SIZE(pWidgets); ++i)
  {
...
nsQtScopedUpdatesDisabled::~nsQtScopedUpdatesDisabled()
{
  for (int i = NS_ARRAY_SIZE(m_pWidgets) - 1; i >= 0; --i)
  {
...
nsQtScopedBlockSignals::nsQtScopedBlockSignals(
  QObject* pObject1, QObject* pObject2, QObject* pObject3, QObject* pObject4, QObject* pObject5, QObject* pObject6)
{
  ...
  for (int i = 0; i < NS_ARRAY_SIZE(pObjects); ++i)
  {
...
nsQtScopedBlockSignals::~nsQtScopedBlockSignals()
{
  for (int i = NS_ARRAY_SIZE(m_pObjects) - 1; i >= 0; --i)
  {
