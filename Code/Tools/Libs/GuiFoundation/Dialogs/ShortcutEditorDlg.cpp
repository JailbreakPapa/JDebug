...
nsQtShortcutEditorDlg::nsQtShortcutEditorDlg(QWidget* pParent)
  : QDialog(pParent)
{
  setupUi(this);

  NS_VERIFY(connect(Shortcuts, SIGNAL(itemSelectionChanged()), this, SLOT(SlotSelectionChanged())) != nullptr, "signal/slot connection failed");

  m_iSelectedAction = -1;
  KeyEditor->setEnabled(false);

  nsMap<nsString, nsMap<nsString, nsInt32>> SortedItems;

  {
    auto itActions = nsActionManager::GetActionIterator();

    while (itActions.IsValid())
    {
      if (itActions.Value()->m_Type == nsActionType::Action)
      {
  ...
  {
    nsQtScopedBlockSignals bs(Shortcuts);
    nsQtScopedUpdatesDisabled ud(Shortcuts);
    ...
    for (auto it = SortedItems.GetIterator(); it.IsValid(); ++it)
    {
      ...
      for (auto it2 : it.Value())
      {
        ...
        sTemp = nsTranslate(item->m_sActionName);
        sTemp.ReplaceAll("&", "");
...
nsQtShortcutEditorDlg::~nsQtShortcutEditorDlg()
{
  nsActionManager::SaveShortcutAssignment();
}

void nsQtShortcutEditorDlg::UpdateTable()
{
  for (nsInt32 iTop = 0; iTop < Shortcuts->topLevelItemCount(); ++iTop)
  {
    auto pTopItem = Shortcuts->topLevelItem(iTop);

    for (nsInt32 iChild = 0; iChild < pTopItem->childCount(); ++iChild)
    {
      auto pChild = pTopItem->child(iChild);

      nsInt32 idx = pChild->data(0, Qt::UserRole).toInt();
...
void nsQtShortcutEditorDlg::SlotSelectionChanged()
{
...
void nsQtShortcutEditorDlg::on_KeyEditor_editingFinished()
{
  UpdateKeyEdit();
}

void nsQtShortcutEditorDlg::UpdateKeyEdit()
{
  if (m_iSelectedAction < 0)
    return;
...
void nsQtShortcutEditorDlg::on_KeyEditor_keySequenceChanged(const QKeySequence& keySequence)
{
  UpdateKeyEdit();
}

void nsQtShortcutEditorDlg::on_ButtonAssign_clicked()
{
  QString sText = KeyEditor->keySequence().toString(QKeySequence::SequenceFormat::NativeText);
  KeyEditor->clear();
...
void nsQtShortcutEditorDlg::on_ButtonRemove_clicked()
{
  KeyEditor->clear();
...
void nsQtShortcutEditorDlg::on_ButtonReset_clicked()
{
  KeyEditor->clear();
