...
nsQtPropertyEditorExpressionWidget::nsQtPropertyEditorExpressionWidget()
...
void nsQtPropertyEditorExpressionWidget::OnInit()
{
}

void nsQtPropertyEditorExpressionWidget::InternalSetValue(const nsVariant& value)
{
  ...
  else
  {
    ...
    QString newText = nsMakeQString(value.ConvertTo<nsString>());
...
void nsQtPropertyEditorExpressionWidget::on_TextChanged()
{
  BroadcastValueChanged(nsVariant(qtToNsString(m_pWidget->toPlainText())));
}
...
{
  auto& knownTypes = nsExpressionParser::GetKnownTypes();
  auto& builtinFunctions = nsExpressionParser::GetBuiltinFunctions();
    ...
    switch (state)
    {
      ...
      case Identifier:
        if (ch.isSpace() || !(ch.isDigit() || ch.isLetter() || ch == '_'))
        {
          ...
          nsTempHashedString tokenHashed(token.toUtf8().data());
