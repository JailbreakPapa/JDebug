...
class NS_GUIFOUNDATION_DLL nsQtImageCache : public QObject
{
  Q_OBJECT

  NS_DECLARE_SINGLETON(nsQtImageCache);
...
public:
  ...
  const QPixmap* QueryPixmap(const char* szAbsolutePath, QModelIndex index = QModelIndex(), QVariant userData1 = QVariant(),
    QVariant userData2 = QVariant(), nsUInt32* out_pImageID = nullptr);
  ...
  void SetMemoryUsageThreshold(nsUInt64 uiMemoryThreshold) { m_iMemoryUsageThreshold = (nsInt64)uiMemoryThreshold; }
...
private:
  ...
  nsSet<Request> m_Requests;
  bool m_bCacheEnabled;
  bool m_bTaskRunning;
  nsTime m_LastCleanupTime;
  nsInt64 m_iMemoryUsageThreshold;
  nsInt64 m_iCurrentMemoryUsage;
  QPixmap* m_pImageLoading;
  QPixmap* m_pImageUnavailable;
  nsUInt32 m_uiCurImageID;

  struct CacheEntry
  {
    QPixmap m_Pixmap;
    nsTime m_LastAccess;
    nsUInt32 m_uiImageID;

    CacheEntry() { m_uiImageID = 0xFFFFFFFF; }
  };

  nsMap<QString, CacheEntry> m_ImageCache;
  nsMap<QString, QPixmap> m_TypeImages;
};

constexpr static nsUInt32 nsThumbnailSize = 256;

NS_ALWAYS_INLINE QPixmap nsSvgThumbnailToPixmap(const char* szFilePath)
{
  return QIcon(szFilePath).pixmap(QSize(nsThumbnailSize, nsThumbnailSize));
}
