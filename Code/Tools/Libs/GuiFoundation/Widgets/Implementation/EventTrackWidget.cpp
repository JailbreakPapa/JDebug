...
nsQtEventTrackWidget::nsQtEventTrackWidget(QWidget* pParent)
  : QWidget(pParent)
{
...
void nsQtEventTrackWidget::SetData(const nsEventTrackData* pData, double fMinCurveLength)
{
  m_pEditData = pData;
...
void nsQtEventTrackWidget::RecreateSortedData()
{
  ...
  for (nsUInt32 idx = 0; idx < m_pEditData->m_ControlPoints.GetCount(); ++idx)
  {
  ...
  // for (auto& cat : m_Categories)
  //{
  //  cat.m_SortedPoints.Sort([](const nsQtEventTrackWidget::Point& lhs, const nsQtEventTrackWidget::Point& rhs) -> bool
  //  {
...
void nsQtEventTrackWidget::SetScrubberPosition(double fPosition)
{
...
void nsQtEventTrackWidget::FrameCurve()
{
  ...
  else if (m_SelectedPoints.GetCount() > 1)
  {
    ...
    for (const auto& cpSel : m_SelectedPoints)
    {
      ...
      minX = nsMath::Min(minX, pos);
      maxX = nsMath::Max(maxX, pos);
    }
...
QPoint nsQtEventTrackWidget::MapFromScene(const QPointF& pos) const
{
...
QPointF nsQtEventTrackWidget::MapToScene(const QPoint& pos) const
{
  double x = pos.x();
  double y = pos.y();
  x /= m_SceneToPixelScale.x();
...
void nsQtEventTrackWidget::ClearSelection()
{
...
void nsQtEventTrackWidget::GetSelection(nsHybridArray<nsUInt32, 32>& out_selection) const
{
  out_selection.Clear();
...
bool nsQtEventTrackWidget::IsSelected(SelectedPoint cp) const
{
  return m_Categories[cp.m_uiCategory].m_SortedPoints[cp.m_uiSortedIdx].m_bSelected;
}

void nsQtEventTrackWidget::SetSelection(SelectedPoint cp)
{
  ClearSelection();
...
void nsQtEventTrackWidget::SetSelection(const nsHybridArray<SelectedPoint, 32>& selection)
{
  for (const auto& cpSel : m_SelectedPoints)
  {
    m_Categories[cpSel.m_uiCategory].m_SortedPoints[cpSel.m_uiSortedIdx].m_bSelected = false;
...
void nsQtEventTrackWidget::ToggleSelected(SelectedPoint cp)
{
...
void nsQtEventTrackWidget::SetSelected(SelectedPoint cp, bool set)
{
...
QRectF nsQtEventTrackWidget::ComputeViewportSceneRect() const
{
...
static nsColorGammaUB g_EventColors[10 * 3] = {
  nsColorGammaUB(255, 102, 0),
  nsColorGammaUB(76, 255, 0),
  nsColorGammaUB(0, 255, 255),
  nsColorGammaUB(239, 35, 0),
  nsColorGammaUB(127, 255, 0),
  nsColorGammaUB(0, 0, 255),
  nsColorGammaUB(205, 92, 92),
  nsColorGammaUB(120, 158, 39),
  nsColorGammaUB(81, 120, 188),
  nsColorGammaUB(255, 105, 180),
  nsColorGammaUB(0, 250, 154),
  nsColorGammaUB(0, 191, 255),
  nsColorGammaUB(220, 20, 60),
  nsColorGammaUB(0, 255, 127),
  nsColorGammaUB(30, 144, 255),
  nsColorGammaUB(240, 128, 128),
  nsColorGammaUB(60, 179, 113),
  nsColorGammaUB(135, 206, 250),
  nsColorGammaUB(178, 34, 34),
  nsColorGammaUB(46, 139, 87),
  nsColorGammaUB(65, 105, 225),
  nsColorGammaUB(211, 122, 122),
  nsColorGammaUB(144, 238, 144),
  nsColorGammaUB(135, 206, 235),
  nsColorGammaUB(219, 112, 147),
  nsColorGammaUB(0, 128, 0),
  nsColorGammaUB(70, 130, 180),
  nsColorGammaUB(255, 182, 193),
  nsColorGammaUB(102, 205, 170),
  nsColorGammaUB(100, 149, 237),
};

void nsQtEventTrackWidget::paintEvent(QPaintEvent* e)
{
...
void nsQtEventTrackWidget::ClampZoomPan()
{
  m_fSceneTranslationX = nsMath::Clamp(m_fSceneTranslationX, -2.0, 50000.0);
...
void nsQtEventTrackWidget::mousePressEvent(QMouseEvent* e)
{
  ...
  if (e->buttons() == Qt::LeftButton) // nothing else pressed
  {
    ...
    if (clickedOn == ClickTarget::Nothing || clickedOn == ClickTarget::SelectedPoint)
    {
      if (e->modifiers() == Qt::NoModifier)
      {
        ...
        {
          case nsQtEventTrackWidget::SelectArea::None:
            break;
          case nsQtEventTrackWidget::SelectArea::Center:
            m_State = EditState::DraggingPoints;
            m_TotalPointDrag = QPointF();
            break;
          case nsQtEventTrackWidget::SelectArea::Left:
            m_State = EditState::ScaleLeftRight;
            m_ScaleReferencePoint = m_SelectionBRect.topRight();
            break;
          case nsQtEventTrackWidget::SelectArea::Right:
            m_State = EditState::ScaleLeftRight;
...
void nsQtEventTrackWidget::mouseReleaseEvent(QMouseEvent* e)
{
  ...
  if (m_State != EditState::MultiSelect && m_pRubberband)
  {
    ...
    {
      nsHybridArray<SelectedPoint, 32> newSelection = m_SelectedPoints;
      nsHybridArray<SelectedPoint, 32> change;
...
void nsQtEventTrackWidget::mouseMoveEvent(QMouseEvent* e)
{
...
void nsQtEventTrackWidget::mouseDoubleClickEvent(QMouseEvent* e)
{
...
void nsQtEventTrackWidget::wheelEvent(QWheelEvent* e)
{
...
void nsQtEventTrackWidget::keyPressEvent(QKeyEvent* e)
{
...
void nsQtEventTrackWidget::PaintOutsideAreaOverlay(QPainter* painter) const
{
  const int iRightEdge = MapFromScene(QPointF(nsMath::Max(0.0, m_fMaxCurveExtent), 0)).x();
...
void nsQtEventTrackWidget::PaintControlPoints(QPainter* painter) const
{
  ...
  nsHybridArray<QLineF, 100> lines;
  nsHybridArray<QRectF, 100> rects;
  nsHybridArray<QLineF, 100> linesSelected;
  nsHybridArray<QRectF, 100> rectsSelected;
  ...
  for (nsUInt32 catIdx = 0; catIdx < m_Categories.GetCount(); ++catIdx)
  {
    ...
    if (!rects.IsEmpty())
    {
      const nsColorGammaUB& col = g_EventColors[catIdx % NS_ARRAY_SIZE(g_EventColors)];
...
void nsQtEventTrackWidget::PaintMultiSelectionSquare(QPainter* painter) const
{
...
void nsQtEventTrackWidget::PaintScrubber(QPainter& p) const
{
  if (!m_bShowScrubber)
    return;
  ...
  const nsInt32 xPos = MapFromScene(QPointF(m_fScrubberPosition, 0)).x();
...
void nsQtEventTrackWidget::RenderVerticalGrid(QPainter* painter, const QRectF& viewportSceneRect, double fRoughGridDensity)
{
  double lowX, highX;
...
bool nsQtEventTrackWidget::PickCpAt(const QPoint& pos, float fMaxPixelDistance, SelectedPoint& out_Result) const
{
  ...
  for (nsUInt32 catIdx = 0; catIdx < m_Categories.GetCount(); ++catIdx)
  {
    ...
    if (nsMath::Abs(diffY) < 20)
    {
      const auto& allPoints = cat.m_SortedPoints;

      for (nsUInt32 ptIdx = 0; ptIdx < allPoints.GetCount(); ++ptIdx)
      {
        const auto& point = allPoints[ptIdx];

        double ptPosX = MapFromScene(QPointF(point.m_fPosX, 0)).x();
        const nsVec2 fDiff(ptPosX - pos.x(), diffY);
...
nsQtEventTrackWidget::ClickTarget nsQtEventTrackWidget::DetectClickTarget(const QPoint& pos)
{
  SelectedPoint ptSel;
...
void nsQtEventTrackWidget::ExecMultiSelection(nsHybridArray<SelectedPoint, 32>& out_Selection)
{
  out_Selection.Clear();
...
bool nsQtEventTrackWidget::CombineSelection(
  nsHybridArray<SelectedPoint, 32>& inout_Selection, const nsHybridArray<SelectedPoint, 32>& change, bool add)
{
  bool bChange = false;
...
void nsQtEventTrackWidget::ComputeSelectionRect()
{
  m_SelectionBRect = QRectF();
  ...
  //  bbox.ExpandToInclude(nsVec3(pos, cpSel.m_uiCategory - 1, 0));
  //  bbox.ExpandToInclude(nsVec3(pos, cpSel.m_uiCategory + 1, 0));
  //}
...
nsQtEventTrackWidget::SelectArea nsQtEventTrackWidget::WhereIsPoint(QPoint pos) const
{
  if (m_SelectionBRect.isEmpty())
