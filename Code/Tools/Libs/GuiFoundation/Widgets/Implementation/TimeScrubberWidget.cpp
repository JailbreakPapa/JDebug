...
nsQtTimeScrubberWidget::nsQtTimeScrubberWidget(QWidget* pParent)
  : QWidget(pParent)
{
  setMinimumWidth(100);
  setMinimumHeight(24);
...
nsQtTimeScrubberWidget::~nsQtTimeScrubberWidget() = default;

void nsQtTimeScrubberWidget::SetDuration(nsUInt64 uiNumTicks)
{
  if (m_uiDurationTicks == uiNumTicks)
    return;

  m_uiDurationTicks = uiNumTicks;
  m_Duration = nsTime::MakeFromSeconds((double)uiNumTicks / 4800.0);
  m_fNormScrubberPosition = nsMath::Clamp((double)m_uiScrubberTickPos / (double)m_uiDurationTicks, 0.0, 1.0);

  update();
}

void nsQtTimeScrubberWidget::SetDuration(nsTime time)
{
  SetDuration(static_cast<nsUInt64>(time.GetSeconds() * 4800));
}

void nsQtTimeScrubberWidget::SetScrubberPosition(nsUInt64 uiTick)
{
  if (m_uiScrubberTickPos == uiTick)
    return;

  m_uiScrubberTickPos = uiTick;
  m_fNormScrubberPosition = nsMath::Clamp((double)uiTick / (double)m_uiDurationTicks, 0.0, 1.0);

  update();
}

void nsQtTimeScrubberWidget::SetScrubberPosition(nsTime time)
{
  SetScrubberPosition(static_cast<nsUInt64>(time.GetSeconds() * 4800));
}

void nsQtTimeScrubberWidget::paintEvent(QPaintEvent* event)
{
  ...
  nsWidgetUtils::AdjustGridDensity(fFineGridDensity, fRoughGridDensity, rect().width(), fMaxDuration, 20);
...
void nsQtTimeScrubberWidget::mousePressEvent(QMouseEvent* event)
{
...
void nsQtTimeScrubberWidget::mouseReleaseEvent(QMouseEvent* event)
{
...
void nsQtTimeScrubberWidget::mouseMoveEvent(QMouseEvent* event)
{
...
void nsQtTimeScrubberWidget::SetScrubberPosFromPixelCoord(nsInt32 posX)
{
  double fNormPos = (double)posX / (double)rect().width();
  fNormPos = nsMath::Clamp(fNormPos, 0.0, 1.0);

  const nsUInt64 uiTickPos = (nsUInt64)(fNormPos * m_uiDurationTicks);
...
nsQtTimeScrubberToolbar::nsQtTimeScrubberToolbar(QWidget* pParent)
  : QToolBar("Time Scrubber", pParent)
{
  m_pScrubber = new nsQtTimeScrubberWidget(this);
...
void nsQtTimeScrubberToolbar::SetDuration(nsUInt64 uiNumTicks)
{
  ...
  nsQtScopedBlockSignals _1(m_pDuration);

  const double oldVal = m_pDuration->text().toDouble();
  const double newVal = (double)uiNumTicks / 4800.0;

  if (nsMath::IsEqual(oldVal, newVal, 0.01))
    return;

  m_pDuration->setText(QString("%1").arg(newVal, 0, 'f', 2));
}

void nsQtTimeScrubberToolbar::SetScrubberPosition(nsUInt64 uiTick)
{
  m_pScrubber->SetScrubberPosition(uiTick);
}

void nsQtTimeScrubberToolbar::SetButtonState(bool bPlaying, bool bRepeatEnabled)
{
  if (bPlaying)
