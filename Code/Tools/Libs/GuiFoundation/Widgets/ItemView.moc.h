...
/// \brief In combination with nsQtItemView this delegate allows for receiving the full range of mouse input.
class NS_GUIFOUNDATION_DLL nsQtItemDelegate : public QItemDelegate
{
  Q_OBJECT
public:
  explicit nsQtItemDelegate(QObject* pParent = nullptr)
...
/// \brief Template to be used with classes derived from QAbstractItemView. Allows the use of nsQtItemDelegate.
template <typename Base>
class nsQtItemView : public Base
{
public:
  nsQtItemView(QWidget* pParent)
    : Base(pParent)
...
private:
  bool ForwardEvent(QModelIndex index, QEvent* pEvent)
  {
    ...
    if (nsQtItemDelegate* pDelegate = qobject_cast<nsQtItemDelegate*>(this->itemDelegateForIndex(m_Hovered)))
    {
...
private:
  nsQtItemDelegate* m_pFocusedDelegate = nullptr;
