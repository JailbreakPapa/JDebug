...
class TestReader : public nsJSONParser
{
private:
  nsDeque<ParseResult> m_Results;
...
public:
  ...
  ~TestReader()
  {
    NS_TEST_INT(m_iExpectedParsingErrors, 0);
    NS_TEST_INT(m_Results.GetCount(), 0);
  }

  void ParseStream(nsStreamReader& inout_stream)
  {
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == Variable);
    NS_TEST_STRING(m_Results.PeekFront().m_szValue, sVarName);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == ValueString);
    NS_TEST_STRING(m_Results.PeekFront().m_szValue, sValue);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == ValueDouble);
    NS_TEST_DOUBLE(m_Results.PeekFront().m_fValue, fValue, 0.0001);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == ValueBool);
    NS_TEST_BOOL(m_Results.PeekFront().m_bValue == bValue);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == ValueNULL);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == BeginObject);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == EndObject);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == BeginArray);
  ...
  {
    NS_TEST_BOOL(!m_Results.IsEmpty());
    NS_TEST_BOOL(m_Results.PeekFront().m_Function == EndArray);

    m_Results.PopFront();
  }

  nsInt32 m_iExpectedParsingErrors;
  ...
  virtual void OnParsingError(nsStringView sMessage, bool bFatal, nsUInt32 uiLine, nsUInt32 uiColumn) override
  {
    ...
    nsStringBuilder tmp;
    NS_TEST_FAILURE("JSON Parsing Error", "(%u, %u): %s", uiLine, uiColumn, sMessage.GetData(tmp));
  }
...
NS_CREATE_SIMPLE_TEST(IO, JSONParser)
{
  NS_TEST_BLOCK(nsTestBlock::Enabled, "All Features")
  {
...
\"test\" : \"text\"\n\
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Empty Document (string")
  {
    const char* szTestData = "";
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Empty Document (Whitespace)")
  {
    const char* szTestData = " \n  \t ";
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Empty Document")
  {
    const char* szTestData = "{}";
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Two Empty Documents")
  {
    const char* szTestData = "{}{}";
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "No Whitespace")
  {
    ...
    NS_TEST_INT(reader.m_iExpectedParsingErrors, 0);
  }

  NS_TEST_BLOCK(nsTestBlock::Enabled, "Empty Blocks")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Superfluous separators")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Too many commas in array")
  {
    // I want C++ 11 raw string literals
    ...
    NS_TEST_INT(reader.m_iExpectedParsingErrors, 0); // must fail to parse
  }

  NS_TEST_BLOCK(nsTestBlock::Enabled, "Comments")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Skip Object")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Skip Array")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Skip Variable")
  {
    // I want C++ 11 raw string literals
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Unicode escape sequence")
  {
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Unicode escape sequence parsing errors")
  {
    const char* szTestData = "{\"a\":\"\\u006G\",\"b\":\"\\u03f\",\"c\":\"a\\ud83ns\",\"d\":\"\\ud83e\t\"}";
