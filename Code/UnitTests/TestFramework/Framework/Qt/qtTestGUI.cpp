...
////////////////////////////////////////////////////////////////////////
// nsQtTestGUI public functions
////////////////////////////////////////////////////////////////////////

nsQtTestGUI::nsQtTestGUI(nsQtTestFramework& ref_testFramework)
  : QMainWindow()
  , m_pTestFramework(&ref_testFramework)
{
  ...
  QCoreApplication::setApplicationName("nsTestFramework");
  ...
  // Model
  m_pModel = new nsQtTestModel(this, m_pTestFramework);
  testTreeView->setModel(m_pModel);

  // Delegate
  m_pDelegate = new nsQtTestDelegate(this);
  ...
  // Message Log Dock
  m_pMessageLogDock = new nsQtLogMessageDock(this, &m_pTestFramework->GetTestResult());
...
void nsQtTestGUI::closeEvent(QCloseEvent* e)
{
...
////////////////////////////////////////////////////////////////////////
// nsQtTestGUI public slots
////////////////////////////////////////////////////////////////////////

void nsQtTestGUI::on_actionAssertOnTestFail_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void nsQtTestGUI::on_actionOpenHTMLOutput_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void nsQtTestGUI::on_actionKeepConsoleOpen_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void nsQtTestGUI::on_actionShowMessageBox_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void nsQtTestGUI::on_actionDisableSuccessfulTests_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void nsQtTestGUI::on_actionSaveTestSettingsAs_triggered()
{
  nsStringBuilder tmp;
  nsStringView defaultDir = nsPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
...
void nsQtTestGUI::on_actionSaveTestOrderAs_triggered()
{
  nsStringBuilder tmp;
  nsStringView defaultDir = nsPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
  QString dir = defaultDir.IsValid() ? defaultDir.GetData(tmp) : QString();
...
void nsQtTestGUI::on_actionRunTests_triggered()
{
  ...
  while (m_pTestFramework->RunTestExecutionLoop() == nsTestAppRun::Continue)
  {
    UpdateButtonStates();
...
void nsQtTestGUI::on_actionAbort_triggered()
{
  m_bAbort = true;
}

void nsQtTestGUI::on_actionQuit_triggered()
{
  close();
}

void nsQtTestGUI::on_actionEnableOnlyThis_triggered()
{
...
void nsQtTestGUI::on_actionEnableOnlyFailed_triggered()
{
  m_pTestFramework->SetAllFailedTestsEnabledStatus();
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void nsQtTestGUI::on_actionEnableAllChildren_triggered()
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
  if (!CurrentIndex.isValid())
    return;
...
void nsQtTestGUI::on_actionEnableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(true);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void nsQtTestGUI::on_actionDisableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(false);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void nsQtTestGUI::on_actionExpandAll_triggered()
{
  testTreeView->expandAll();
}

void nsQtTestGUI::on_actionCollapseAll_triggered()
{
  testTreeView->collapseAll();
}

void nsQtTestGUI::onTestFrameworkTestResultReceived(qint32 iTestIndex, qint32 iSubTestIndex)
{
  ...
  // Update status bar
  const nsUInt32 uiTestCount = m_uiTestsEnabledCount;
  const nsUInt32 uiFailed = m_pTestFramework->GetTestsFailedCount();
  const nsUInt32 uiPassed = m_pTestFramework->GetTestsPassedCount();
  const nsUInt32 uiErrors = m_pTestFramework->GetTotalErrorCount();
  ...
  {
    fSubTestPercentage =
      (float)m_pTestFramework->GetTestResult().GetSubTestCount(m_pTestFramework->GetCurrentTestIndex(), nsTestResultQuery::Executed) /
...
void nsQtTestGUI::onTestTreeViewCustomContextMenuRequested(const QPoint& pnt)
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
...
void nsQtTestGUI::onSelectionModelCurrentRowChanged(const QModelIndex& index)
{
  if (!index.isValid())
  {
  ...
  const nsTestResultData* pTestResult = pEntry->GetTestResult();
...
void nsQtTestGUI::on_actionOpenTestDataFolder_triggered()
{
...
void nsQtTestGUI::on_actionOpenOutputFolder_triggered()
{
...
void nsQtTestGUI::on_actionOpenHTMLFile_triggered()
{
  std::string sOutputFile = std::string(nsTestFramework::GetInstance()->GetAbsOutputPath()) + "/UnitTestsLog.htm";

  QDesktopServices::openUrl(QUrl::fromLocalFile(sOutputFile.c_str()));
}

void nsQtTestGUI::on_actionUpdateReferenceImages_triggered()
{
  m_pTestFramework->UpdateReferenceImages();
}

////////////////////////////////////////////////////////////////////////
// nsQtTestGUI private functions
////////////////////////////////////////////////////////////////////////

void nsQtTestGUI::UpdateButtonStates()
{
...
void nsQtTestGUI::SaveGUILayout()
{
  QSettings Settings;
...
void nsQtTestGUI::LoadGUILayout()
{
  QSettings Settings;
  Settings.beginGroup("MainWindow");
...
void nsQtTestGUI::SetCheckStateRecursive(const QModelIndex& index, bool bChecked)
{
  m_pModel->setData(index, bChecked ? Qt::Checked : Qt::Unchecked, Qt::CheckStateRole);

  nsInt32 iChildren = m_pModel->rowCount(index);
...
void nsQtTestGUI::EnableAllParents(const QModelIndex& index)
{
...
////////////////////////////////////////////////////////////////////////
// nsQtTestGUI public static functions
////////////////////////////////////////////////////////////////////////

void nsQtTestGUI::SetDarkTheme()
{
  // return;
