...
struct nsOutputToHTML
{
  static std::ofstream htmlFile;

  static void OutputToHTML(nsTestOutput::Enum type, const char* szMsg)
  {
    if (type != nsTestOutput::StartOutput && !htmlFile.is_open())
      return;
    ...
    switch (type)
    {
      case nsTestOutput::StartOutput:
      {
        ...
        nsTestFramework::GetInstance()->CreateOutputFolder();

        std::string sOutputFile = std::string(nsTestFramework::GetInstance()->GetAbsOutputPath()) + "/UnitTestsLog.htm";
        const char* szTestName = nsTestFramework::GetInstance()->GetTestName();
...
#if NS_ENABLED(NS_PLATFORM_WINDOWS_DESKTOP)
        TestSettings settings = nsTestFramework::GetInstance()->GetSettings();
...
std::ofstream nsOutputToHTML::htmlFile;
