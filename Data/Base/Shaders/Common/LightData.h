...
struct WD_SHADER_STRUCT wdPerLightData
{
  UINT1(colorAndType);
  FLOAT1(intensity);
  ...
  StructuredBuffer<wdPerLightData> perLightDataBuffer;
#else
  WD_CHECK_AT_COMPILETIME(sizeof(wdPerLightData) == 48);
#endif

struct WD_SHADER_STRUCT wdPointShadowData
{
  FLOAT4(shadowParams); // x = slope bias, y = constant bias, z = penumbra size in texel, w = fadeout
  MAT4(worldToLightMatrix)[6];
};

struct WD_SHADER_STRUCT wdSpotShadowData
{
  FLOAT4(shadowParams); // x = slope bias, y = constant bias, z = penumbra size in texel, w = fadeout
  MAT4(worldToLightMatrix);
};

struct WD_SHADER_STRUCT wdDirShadowData
{
...
struct WD_SHADER_STRUCT wdPerDecalData
{
...
#if WD_ENABLED(PLATFORM_SHADER)
  StructuredBuffer<wdPerDecalData> perDecalDataBuffer;
#else // C++
  WD_CHECK_AT_COMPILETIME(sizeof(wdPerDecalData) == 96);
#endif
...
#define GET_REFLECTION_PROBE_INDEX(index) (index & REFLECTION_PROBE_INDEX_BITMASK)
  ...
  struct WD_SHADER_STRUCT wdPerReflectionProbeData
  {
...
#if WD_ENABLED(PLATFORM_SHADER)
  StructuredBuffer<wdPerReflectionProbeData> perPerReflectionProbeDataBuffer;
#else // C++
  WD_CHECK_AT_COMPILETIME(sizeof(wdPerReflectionProbeData) == 160);
#endif

  CONSTANT_BUFFER(wdClusteredDataConstants, 3)
  {
...
#if WD_ENABLED(PLATFORM_SHADER)
  StructuredBuffer<wdPerClusterData> perClusterDataBuffer;
