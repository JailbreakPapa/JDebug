...
nsPerClusterData GetClusterData(float3 screenPosition)
{
...
float3 ComputeReflection(inout nsMaterialData matData, float3 viewVector, nsPerClusterData clusterData)
{
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    const nsPerReflectionProbeData probeData = perPerReflectionProbeDataBuffer[probeIndex];
...
AccumulatedLight CalculateLighting(nsMaterialData matData, nsPerClusterData clusterData, float3 screenPosition, bool applySSAO)
{
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    nsPerLightData lightData = perLightDataBuffer[lightIndex];
...
void ApplyDecals(inout nsMaterialData matData, nsPerClusterData clusterData, uint gameObjectId)
{
  uint firstItemIndex = clusterData.offset;
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    nsPerDecalData decalData = perDecalDataBuffer[decalIndex];
...
void ApplyRefraction(inout nsMaterialData matData, inout AccumulatedLight light)
{
