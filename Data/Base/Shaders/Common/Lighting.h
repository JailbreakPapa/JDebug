...
wdPerClusterData GetClusterData(float3 screenPosition)
{
...
float CalculateShadowTerm(wdMaterialData matData, float3 lightVector, float distanceToLight, uint type,
...
float3 ComputeReflection(inout wdMaterialData matData, float3 viewVector, wdPerClusterData clusterData)
{
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    const wdPerReflectionProbeData probeData = perPerReflectionProbeDataBuffer[probeIndex];
...
AccumulatedLight CalculateLighting(wdMaterialData matData, wdPerClusterData clusterData, float3 screenPosition, bool applySSAO)
{
  ...
  for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    wdPerLightData lightData = perLightDataBuffer[lightIndex];
...
void ApplyDecals(inout wdMaterialData matData, wdPerClusterData clusterData, uint gameObjectId)
{
  uint firstItemIndex = clusterData.offset;
  ...
  for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    wdPerDecalData decalData = perDecalDataBuffer[decalIndex];
...
void ApplyRefraction(inout wdMaterialData matData, inout AccumulatedLight light)
{
