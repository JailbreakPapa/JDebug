...
float3 GetObjectPositionOffset(nsPerInstanceData data);
#endif

#if defined(USE_WORLD_POSITION_OFFSET)
float3 GetWorldPositionOffset(nsPerInstanceData data, float3 worldPosition);
#endif
...
VS_OUT FillVertexData(VS_IN Input)
{
  ...
  nsPerInstanceData data = GetInstanceData();
