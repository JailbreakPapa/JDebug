...
float3 GetObjectPositionOffset(wdPerInstanceData data);
#endif

#if defined(USE_WORLD_POSITION_OFFSET)
float3 GetWorldPositionOffset(wdPerInstanceData data, float3 worldPosition);
#endif
...
VS_OUT FillVertexData(VS_IN Input)
{
  ...
  wdPerInstanceData data = GetInstanceData();
