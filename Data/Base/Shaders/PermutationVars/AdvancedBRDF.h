...
{
  // Estevns and Kulla 2017, "Production Friendly Microfacet Sheen BRDF"
...
float3 Diffuse_Lambert(nsMaterialData matData, float NoV, float NoL, float VoH, float NdotH)
{
  return matData.diffuseColor;
}

// [Burley 2012, "Physically-Based Shading at Disney"]
float3 Diffuse_Burley(nsMaterialData matData, float NoV, float NoL, float VoH, float NdotH)
{
...
float3 Diffuse_OrenNayar(nsMaterialData matData, float NoV, float NoL, float VoH, float NdotH)
{
...
float3 Diffuse_Chan(nsMaterialData matData, float NoV, float NoL, float VoH, float NoH)
{
  NoV = saturate(NoV);
  NoL = saturate(NoL);
  VoH = saturate(VoH);
...
float3 BRDF_Diffuse(nsMaterialData matData, float NoV, float NoL, float VoH, float NoH)
{
...
float3 BRDF_Specular_Anisotropic(nsMaterialData matData, float3 h, float NoV, float NoL, float NoH, float LoH)
{
...
float3 BRDF_Specular_Clearcoat(nsMaterialData matData, float NdotH, float VdotH)
{
...
float3 BRDF_Specular_Sheen(nsMaterialData matData, float NdotV, float NdotL, float NdotH)
{
...
AccumulatedLight DefaultShadingNew(nsMaterialData matData, float3 L, float3 V)
{
