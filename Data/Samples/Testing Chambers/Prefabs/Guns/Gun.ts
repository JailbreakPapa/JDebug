...
export class MsgGunInteraction extends wd.Message {
    WD_DECLARE_MESSAGE_TYPE;

    interaction: GunInteraction;
    keyState: wd.TriggerState;
    ammoPouch: AmmoPouch = null;
}

export abstract class Gun extends wd.TickedTypescriptComponent {
    ...
    shootSoundComponent: wd.FmodEventComponent = null;
    muzzleFlashComponent: wd.ParticleComponent = null;

    static RegisterMessageHandlers() {

        wd.TypescriptComponent.RegisterMessageHandler(MsgGunInteraction, "OnMsgGunInteraction");
    }
    ...
    OnSimulationStarted(): void {
        ...
        if (node != null) {
            this.shootSoundComponent = node.TryGetComponentOfBaseType(wd.FmodEventComponent);
        }

        node = owner.FindChildByName("Muzzleflash", true);
        if (node != null) {
            this.muzzleFlashComponent = node.TryGetComponentOfBaseType(wd.ParticleComponent);
        }
    }
    ...
    OnMsgGunInteraction(msg: MsgGunInteraction): void {
        ...
        if (msg.interaction == GunInteraction.Fire) {
            ...
            if (msg.keyState == wd.TriggerState.Deactivated) {
            ...
            if (this.singleShotPerTrigger) {
                ...
                if (msg.keyState == wd.TriggerState.Activated) {
