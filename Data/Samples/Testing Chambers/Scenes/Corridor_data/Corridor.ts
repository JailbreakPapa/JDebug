...
export class Corridor extends wd.TickedTypescriptComponent {
    ...
    OnSimulationStarted(): void {
        this.SetTickInterval(wd.Time.Milliseconds(1000));
    }
    ...
    OnMsgGenericEvent(msg: wd.MsgGenericEvent): void {
        ...
        if (msg.Message == "SecretDoorButton") {
            ...
            let door = wd.World.TryGetObjectWithGlobalKey("SecretDoor");
            if (door != null) {

                let slider = door.TryGetComponentOfBaseType(wd.SliderComponent);
        ...
        if (msg.Message == "MoveA" || msg.Message == "MoveB") {
            let obj = wd.World.TryGetObjectWithGlobalKey("Obj");

            let move = obj.TryGetComponentOfBaseType(wd.MoveToComponent);

            move.Running = true;

            if (msg.Message == "MoveA") {
                move.SetTargetPosition(new wd.Vec3(10, -1, 1.5));
            }
            else {
                move.SetTargetPosition(new wd.Vec3(10, 3, 1.5));
            }
        }
        ...
        if (msg.Message == "SwitchMonitor1") {
            ...
            let mon = wd.Utils.FindPrefabRootScript<monitor.Monitor>(wd.World.TryGetObjectWithGlobalKey("Monitor1"), "Monitor");
