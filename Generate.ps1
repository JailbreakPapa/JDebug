...
if ($NoUnityBuild) {
    $CMAKE_ARGS += "-DWD_ENABLE_FOLDER_UNITY_FILES:BOOL=OFF"
}
else {
    $CMAKE_ARGS += "-DWD_ENABLE_FOLDER_UNITY_FILES:BOOL=ON"
}

if ($SolutionName -ne "") {
    $CMAKE_ARGS += "-DWD_SOLUTION_NAME:STRING='${SolutionName}'"
}
...
elseif ($Target -eq "Uwp64vs2019") {
    ...
    $CMAKE_ARGS += "-DWD_ENABLE_QT_SUPPORT:BOOL=OFF"
    $CMAKE_ARGS += "-DWD_BUILD_FILTER='UwpProjects'"
}
